...
## Chapter 1. Introduction[](https://docs.freebsd.org/en/books/porters-handbook/book/#why-port)
...
## Chapter 2. Making a New Port[](https://docs.freebsd.org/en/books/porters-handbook/book/#own-port)
...
## Chapter 3. Quick Porting[](https://docs.freebsd.org/en/books/porters-handbook/book/#quick-porting)
...
### 4.2. Getting the Original Sources[](https://docs.freebsd.org/en/books/porters-handbook/book/#slow-sources)
...
### 4.5. Configuring[](https://docs.freebsd.org/en/books/porters-handbook/book/#slow-configure)
...
#### 5.2.2. Versions, `DISTVERSION` *or* `PORTVERSION`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-versions)
...
#### 5.3.3. Choosing the Right Category[](https://docs.freebsd.org/en/books/porters-handbook/book/#choosing-categories)
...
#### 5.3.4. Proposing a New Category[](https://docs.freebsd.org/en/books/porters-handbook/book/#proposing-categories)
...
#### 5.3.5. Proposing Reorganizing All the Categories[](https://docs.freebsd.org/en/books/porters-handbook/book/#proposing-reorg)
...
### 5.4. The Distribution Files[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-distfiles)
...
#### 5.4.1. `DISTNAME`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-distname)
...
In [comms/librs232](https://cgit.freebsd.org/ports/tree/comms/librs232/), the distribution file is not versioned, so using [`DIST_SUBDIR`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-dist_subdir) is needed:
...
#### 5.4.2. `MASTER_SITES`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites)
...
##### 5.4.2.1. Using `MASTER_SITE_*` Variables[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-shorthand)
...
|     | For any `MASTER_SITE_FOO` variable, the shorthand `*FOO*` can be used. For example, use:<br><br>MASTER_SITES=    FOO<br><br>If `MASTER_SITE_SUBDIR` is needed, use this:<br><br>MASTER_SITES=    FOO/bar |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
##### 5.4.2.2. Magic MASTER_SITES Macros[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-magic)
...
#### 5.4.3. `USE_GITHUB`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-github)
...
While trying to make a port for the bleeding edge version of pkg from the FreeBSD user on github, at [GitHub - freebsd/pkg: Package management tool for FreeBSD. Help at #pkg on Libera Chat or pkg@FreeBSD.org](https://github.com/freebsd/pkg/), the Makefile ends up looking like this (slightly stripped for the example):
...
If there never was a version upstream, do not invent one like `0.1` or `1.0`. Create the port with a `DISTVERSION` of `g*YYYYMMDD*`, where `g` is for Git, and `*YYYYMMDD*` represents the date the commit referenced in `GH_TAGNAME`.
...
##### 5.4.3.1. Fetching Multiple Files from GitHub[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-github-multiple)
...
|     | When using `USE_GITHUB=nodefault`, the Makefile must set `DISTFILES` in its [top block](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-portname). The definition should be:<br><br>DISTFILES=    ${DISTNAME}${EXTRACT_SUFX} |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
This is functionally equivalent to [Use of `USE_GITHUB` with Multiple Distribution Files](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-github-multi), but using `GH_TUPLE`:
...
Using [finance/moneymanagerex](https://cgit.freebsd.org/ports/tree/finance/moneymanagerex/) as an example, its GitHub repository is [GitHub - moneymanagerex/moneymanagerex: Money Manager Ex is an easy to use, money management application built with wxWidgets](https://github.com/moneymanagerex/moneymanagerex/). It has a [.gitmodules](https://github.com/moneymanagerex/moneymanagerex/blob/master/.gitmodules) file at the root. This file describes all the submodules used in this repository, and lists additional repositories needed. This file will tell what additional repositories are needed:
...
#### 5.4.4. `USE_GITLAB`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-gitlab)
...
While trying to make a port for version `1.14` of libsignon-glib from the accounts-sso user on gitlab.com, at [Accounts SSO / libsignon-glib · GitLab](https://gitlab.com/accounts-sso/libsignon-glib/), The Makefile would end up looking like this for fetching the distribution files:
...
##### 5.4.4.1. Fetching Multiple Files from GitLab[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-gitlab-multiple)
...
Multiple values are added to `GL_SITE`, `GL_ACCOUNT`, `GL_PROJECT` and `GL_COMMIT`. Each different value is assigned a group. [`USE_GITLAB` Description](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-gitlab-description).
...
|     | When using `USE_GITLAB=nodefault`, the Makefile must set `DISTFILES` in its [top block](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-portname). The definition should be:<br><br>DISTFILES=    ${DISTNAME}${EXTRACT_SUFX} |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
This is functionally equivalent to [Use of `USE_GITLAB` with Multiple Distribution Files](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-gitlab-multi), but using `GL_TUPLE`:
...
#### 5.4.5. `EXTRACT_SUFX`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-extract_sufx)
...
The `USES=tar[:*xxx*]`, `USES=lha` or `USES=zip` automatically set `EXTRACT_SUFX` to the most common archives extensions as necessary, see [Using `USES` Macros](https://docs.freebsd.org/en/books/porters-handbook/book/#uses) for more details. If neither of these are set then `EXTRACT_SUFX` defaults to `.tar.gz`.

|     | As `EXTRACT_SUFX` is only used in `DISTFILES`, only set one of them.. |
| --- | --------------------------------------------------------------------- |

#### 5.4.6. `DISTFILES`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-distfiles-definition)
...
#### 5.4.7. `EXTRACT_ONLY`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-extract_only)
...
#### 5.4.8. `PATCHFILES`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-patchfiles)
...
|     | If there are multiple patches and they need mixed values for the strip parameter, it can be added alongside the patch name in `PATCHFILES`, e.g:<br><br>PATCHFILES=    patch1 patch2:-p1<br><br>This does not conflict with [the master site grouping feature](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-master-sites-n), adding a group also works:<br><br>PATCHFILES=    patch2:-p1:source2 |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
#### 5.4.9. Multiple Distribution or Patches Files from Multiple Locations[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-master-sites-n)
...
##### 5.4.9.1. Simplified Information[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-master-sites-n-simplified)
...
##### 5.4.9.2. Detailed Information[](https://docs.freebsd.org/en/books/porters-handbook/book/#ports-master-sites-n-detailed)
...
8. How do I group one of the special macros from bsd.sites.mk, for example, SourceForge (`SF`)?
   ...
   This has been simplified as much as possible. See [Detailed Use of `MASTER_SITES:n` with SourceForge (`SF`)](https://docs.freebsd.org/en/books/porters-handbook/book/#ports-master-sites-n-example-detailed-use-master-site-sourceforge).
...
9. How do I use this with `PATCH*`?
   ...
   All examples were done with `MASTER*` but they work exactly the same for `PATCH*` ones as can be seen in [Simplified Use of `MASTER_SITES:n` with `PATCH_SITES`](https://docs.freebsd.org/en/books/porters-handbook/book/#ports-master-sites-n-example-detailed-use-patch-sites).
...
##### 5.4.9.3. What Does Change for Ports? What Does Not?[](https://docs.freebsd.org/en/books/porters-handbook/book/#port-master-sites-n-what-changed)

1. All current ports remain the same. The `MASTER_SITES:n` feature code is only activated if there are elements postfixed with `:*n*` like elements according to the aforementioned syntax rules, especially as shown in item [7](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-master-sites-n-group-semantics).
...
2. The port targets remain the same: `checksum`, `makesum`, `patch`, `configure`, `build`, etc. With the obvious exceptions of `do-fetch`, `fetch-list`, `master-sites` and `patch-sites`.
    ...
    * `do-fetch`: deploys the new grouping postfixed `DISTFILES` and `PATCHFILES` with their matching group elements within both `MASTER_SITES` and `PATCH_SITES` which use matching group elements within both `MASTER_SITE_SUBDIR` and `PATCH_SITE_SUBDIR`. Check [Detailed Use of `MASTER_SITES:n` with Comma Operator, Multiple Files, Multiple Sites and Multiple Subdirectories](https://docs.freebsd.org/en/books/porters-handbook/book/#ports-master-sites-n-example-detailed-use-complete-example-master-sites).
...
#### 5.4.10. `DIST_SUBDIR`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-dist_subdir)
...
### 5.5. `MAINTAINER`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-maintainer)
...
### 5.6. `COMMENT`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-comment)
...
### 5.7. Project website[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-www)
...
### 5.8. Licenses[](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses)
...
#### 5.8.1. `LICENSE`[](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license)
...
#### 5.8.2. `LICENSE_PERMS` and `LICENSE_PERMS_NAME_`[](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_perms)
...
Redistribution of the distribution files is prohibited. This is equivalent to setting [`RESTRICTED`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-restricted). The distribution files will *not* be added to the FreeBSD `MASTER_SITE_BACKUP` CDN.
...
Selling of distribution files is prohibited. This is equivalent to setting [`NO_CDROM`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-no_cdrom).
...
Free redistribution of package is prohibited. Equivalent to setting [`NO_PACKAGE`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-no_package). The package will *not* be distributed from the FreeBSD package CDN [https://pkg.freebsd.org/](https://pkg.freebsd.org/).
...
Selling of package is prohibited. This is equivalent to setting [`NO_CDROM`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-no_cdrom). The package will *not* be present on the installer images.
...
#### 5.8.3. `LICENSE_GROUPS` and `LICENSE_GROUPS_NAME`[](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_groups)
...
Free Software Foundation Approved, see the [FSF Licensing & Compliance Team](https://www.fsf.org/licensing/).
...
OSI Approved, see the Open Source Initiative [Open Source Licenses](https://opensource.org/licenses/) page.

`COPYFREE`

Comply with Copyfree Standard Definition, see the [Copyfree Licenses](https://copyfree.org/standard/licenses/) page.

`FONTS`

Font licenses

#### 5.8.4. `LICENSE_NAME` and `LICENSE_NAME_NAME`[](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_name)
...
#### 5.8.5. `LICENSE_FILE` and `LICENSE_FILE_NAME`[](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_file)

Full path to the file containing the license text, usually ${WRKSRC}/some/file. If the file is not in the distfile, and its content is too long to be put in [`LICENSE_TEXT`](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_text), put it in a new file in ${FILESDIR}.
...
#### 5.8.6. `LICENSE_TEXT` and `LICENSE_TEXT_NAME`[](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_text)
...
#### 5.8.7. `LICENSE_DISTFILES` and `LICENSE_DISTFILES_NAME`[](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_distfiles)
...
#### 5.8.8. `LICENSE_COMB`[](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_comb)
...
### 5.9. `PORTSCOUT`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-portscout)
...
### 5.10. Dependencies[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-depend)
...
#### 5.10.1. `LIB_DEPENDS`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-lib_depends)
...
#### 5.10.2. `RUN_DEPENDS`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-run_depends)
...
|     | The official search `PATH` used on the ports build cluster is<br><br>/sbin:/bin:/usr/sbin:/usr/bin:/usr/local/sbin:/usr/local/bin |
| --- | --------------------------------------------------------------------------------------------------------------------------------- |
...
#### 5.10.3. `BUILD_DEPENDS`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-build_depends)
...
#### 5.10.4. `FETCH_DEPENDS`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-fetch_depends)
...
#### 5.10.5. `EXTRACT_DEPENDS`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-extract_depends)
...
|     | Use this variable only if the extraction does not already work (the default assumes `tar`) and cannot be made to work using `USES=tar`, `USES=lha` or `USES=zip` described in [Using `USES` Macros](https://docs.freebsd.org/en/books/porters-handbook/book/#uses). |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

#### 5.10.6. `PATCH_DEPENDS`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-patch_depends)
...
#### 5.10.7. `USES`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-uses)
...
For the complete list of values, please see [Using `USES` Macros](https://docs.freebsd.org/en/books/porters-handbook/book/#uses).

|     | `USES` cannot be assigned after inclusion of bsd.port.pre.mk. |
| --- | ------------------------------------------------------------- |

#### 5.10.8. `USE_*`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-use-vars)
...
|     | It is *always* incorrect to set any `USE_*` in /etc/make.conf. For instance, setting<br><br>USE_GCC=X.Y<br><br>(where X.Y is version number) would add a dependency on gccXY for every port, including `lang/gccXY` itself! |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

Table 8. `USE_*`VariableMeans`USE_GCC`
...
#### 5.10.9. Minimal Version of a Dependency[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-version-dependency)
...
#### 5.10.10. Notes on Dependencies[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-note-on-dependencies)
...
#### 5.10.11. Circular Dependencies Are Fatal[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-circular-dependencies)
...
#### 5.10.12. Problems Caused by Automatic Dependencies[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-automatic-dependencies)
...
### 5.11. Slave Ports and `MASTERDIR`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-masterdir)
...
### 5.12. Man Pages[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-manpages)
...
### 5.13. Info Files[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-info)
...
### 5.14. Makefile Options[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-options)
...
#### 5.14.1. `OPTIONS`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-options-options)

##### 5.14.1.1. Background[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-options-background)
...
##### 5.14.1.2. Syntax[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-options-syntax)
...
##### 5.14.1.3. Default Options[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-options-default)
...
#### 5.14.2. Feature Auto-Activation[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-options-auto-activation)
...
|     | Under some circumstances, the shorthand conditional syntax can cause problems with complex constructs. The errors are usually `Malformed conditional`, an alternative syntax can be used.<br><br>.if !empty(VARIABLE:MVALUE)<br><br>as an alternative to<br><br>.if ${VARIABLE:MVALUE} |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

#### 5.14.3. Options Helpers[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-helpers)
...
For automatic `%%*OPT*%%` and `%%NO*OPT*%%` generation, see [`OPTIONS_SUB`](https://docs.freebsd.org/en/books/porters-handbook/book/#options_sub).

For more complex usage, see [Generic Variables Replacement, `OPT_VARIABLE` and `OPT_VARIABLE_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-variables).

`CONFIGURE_ARGS`

For `--enable-*x*` and `--disable-*x*`, see [`OPT_CONFIGURE_ENABLE`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-configure_enable).

For `--with-*x*` and `--without-*x*`, see [`OPT_CONFIGURE_WITH`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-configure_with).

For all other cases, see [`OPT_CONFIGURE_ON` and `OPT_CONFIGURE_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-configure_on).

`CMAKE_ARGS`

For arguments that are booleans (`on`, `off`, `true`, `false`, `0`, `1`) see [`OPT_CMAKE_BOOL` and `OPT_CMAKE_BOOL_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-cmake_bool).

For all other cases, see [`OPT_CMAKE_ON` and `OPT_CMAKE_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-cmake_on).

`MESON_ARGS`

For arguments that take `true` or `false`, see [`OPT_MESON_TRUE` and `OPT_MESON_FALSE`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-meson_true).

For arguments that take `yes` or `no`, use [`OPT_MESON_YES` and `OPT_MESON_NO`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-meson_yes).

For arguments that take `enabled` or `disabled`, see [`OPT_MESON_ENABLED` and `OPT_MESON_DISABLED`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-meson_enabled).

For all other cases, use [`OPT_MESON_ON` and `OPT_MESON_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-meson_on).

`QMAKE_ARGS`

See [`OPT_QMAKE_ON` and `OPT_QMAKE_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-qmake_on).

`USE_*`

See [`OPT_USE` and `OPT_USE_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-use).

`*_DEPENDS`

See [Dependencies, `OPT_DEPTYPE` and `OPT_DEPTYPE_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-dependencies).

`*` (Any variable)

The most used variables have direct helpers, see [Generic Variables Replacement, `OPT_VARIABLE` and `OPT_VARIABLE_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-variables).

For any variable without a specific helper, see [`OPT_VARS` and `OPT_VARS_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-vars).

Options dependencies

When an option need another option to work, see [`OPT_IMPLIES`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-implies).

Options conflicts

When an option cannot work if another is also enabled, see [`OPT_PREVENTS` and `OPT_PREVENTS_MSG`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-prevents).

Build targets

When an option need some extra processing, see [Additional Build Targets, `*target*-*OPT*-on` and `*target*-*OPT*-off`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-targets).

##### 5.14.3.1. `OPTIONS_SUB`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options_sub)
...
##### 5.14.3.2. `OPT_USE` and `OPT_USE_OFF`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-use)
...
##### 5.14.3.3. `CONFIGURE_ARGS` Helpers[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-configure-helpers)

###### 5.14.3.3.1. `OPT_CONFIGURE_ENABLE`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-configure_enable)
...
###### 5.14.3.3.2. `OPT_CONFIGURE_WITH`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-configure_with)
...
###### 5.14.3.3.3. `OPT_CONFIGURE_ON` and `OPT_CONFIGURE_OFF`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-configure_on)
...
|     | Most of the time, the helpers in [`OPT_CONFIGURE_ENABLE`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-configure_enable) and [`OPT_CONFIGURE_WITH`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-configure_with) provide a shorter and more comprehensive functionality. |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

##### 5.14.3.4. `CMAKE_ARGS` Helpers[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-cmake-helpers)

###### 5.14.3.4.1. `OPT_CMAKE_ON` and `OPT_CMAKE_OFF`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-cmake_on)
...
|     | See [`OPT_CMAKE_BOOL` and `OPT_CMAKE_BOOL_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-cmake_bool) for a shorter helper when the value is boolean. |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

###### 5.14.3.4.2. `OPT_CMAKE_BOOL` and `OPT_CMAKE_BOOL_OFF`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-cmake_bool)
...
##### 5.14.3.5. `MESON_ARGS` Helpers[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-meson-helpers)

###### 5.14.3.5.1. `OPT_MESON_ON` and `OPT_MESON_OFF`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-meson_on)
...
###### 5.14.3.5.2. `OPT_MESON_TRUE` and `OPT_MESON_FALSE`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-meson_true)
...
###### 5.14.3.5.3. `OPT_MESON_YES` and `OPT_MESON_NO`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-meson_yes)
...
###### 5.14.3.5.4. `OPT_MESON_ENABLED` and `OPT_MESON_DISABLED`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-meson_enabled)
...
##### 5.14.3.6. `OPT_QMAKE_ON` and `OPT_QMAKE_OFF`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-qmake_on)
...
##### 5.14.3.7. `OPT_IMPLIES`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-implies)

Provides a way to add dependencies between options.

When *OPT* is selected, all the options listed in this variable will be selected too. Using the [`OPT_CONFIGURE_ENABLE`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-configure_enable) described earlier to illustrate:
...
##### 5.14.3.8. `OPT_PREVENTS` and `OPT_PREVENTS_MSG`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-prevents)
...
##### 5.14.3.9. `OPT_VARS` and `OPT_VARS_OFF`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-vars)

Provides a generic way to set and append to variables.

|     | Before using `OPT_VARS` and `OPT_VARS_OFF`, see if there is already a more specific helper available in [Generic Variables Replacement, `OPT_VARIABLE` and `OPT_VARIABLE_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-variables). |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
|     | Values containing whitespace must be enclosed in quotes:<br><br>OPT_VARS=    foo="bar baz"<br><br>This is due to the way [make(1)](https://man.freebsd.org/cgi/man.cgi?query=make&sektion=1&format=html) variable expansion deals with whitespace. When `OPT_VARS= foo=bar baz` is expanded, the variable ends up containing two strings, `foo=bar` and `baz`. But the submitter probably intended there to be only one string, `foo=bar baz`. Quoting the value prevents whitespace from being used as a delimiter.<br><br>Also, *do not* add extra spaces after the `*var*=` sign and before the value, it would also be split into two strings. *This will not work*:<br><br>OPT_VARS=    foo=    bar |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

##### 5.14.3.10. Dependencies, `OPT_DEPTYPE` and `OPT_DEPTYPE_OFF`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-dependencies)
...
##### 5.14.3.11. Generic Variables Replacement, `OPT_VARIABLE` and `OPT_VARIABLE_OFF`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-variables)
...
|     | Some of these variables, at least `ALL_TARGET`, `DISTFILES` and `INSTALL_TARGET`, have their default values set *after* the options are processed.<br><br>With these lines in the Makefile:<br><br>ALL_TARGET=    all<br>DOCS_ALL_TARGET=    doc<br><br>If the `DOCS` option is enabled, `ALL_TARGET` will have a final value of `all doc`; if the option is disabled, it would have a value of `all`.<br><br>With only the options helper line in the Makefile:<br><br>DOCS_ALL_TARGET=    doc<br><br>If the `DOCS` option is enabled, `ALL_TARGET` will have a final value of `doc`; if the option is disabled, it would have a value of `all`. |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

##### 5.14.3.12. Additional Build Targets, `*target*-*OPT*-on` and `*target*-*OPT*-off`[](https://docs.freebsd.org/en/books/porters-handbook/book/#options-targets)
...
### 5.15. Specifying the Working Directory[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-wrkdir)
...
#### 5.15.1. `WRKSRC`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-wrksrc)
...
#### 5.15.2. `WRKSRC_SUBDIR`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-wrksrc_subdir)
...
#### 5.15.3. `NO_WRKSUBDIR`[](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-no_wrksubdir)
...
### 5.16. Conflict Handling[](https://docs.freebsd.org/en/books/porters-handbook/book/#conflicts)
...
|     | The conflict variables automatically set the variable `IGNORE`, which is more fully documented in [Marking a Port Not Installable with `BROKEN`](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall). |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
### 5.17. Installing Files[](https://docs.freebsd.org/en/books/porters-handbook/book/#install)
...
#### 5.17.1. `INSTALL_*` Macros[](https://docs.freebsd.org/en/books/porters-handbook/book/#install-macros)
...
#### 5.17.2. Stripping Binaries and Shared Libraries[](https://docs.freebsd.org/en/books/porters-handbook/book/#install-strip)
...
#### 5.17.3. Installing a Whole Tree of Files[](https://docs.freebsd.org/en/books/porters-handbook/book/#install-copytree)
...
#### 5.17.4. Install Additional Documentation[](https://docs.freebsd.org/en/books/porters-handbook/book/#install-documentation)
...
On the other hand, if there is a DOCS option in the port, install the documentation in a `post-install-DOCS-on` target. These targets are described in [Additional Build Targets, `*target*-*OPT*-on` and `*target*-*OPT*-off`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-targets).
...
#### 5.17.5. Subdirectories Under `PREFIX`[](https://docs.freebsd.org/en/books/porters-handbook/book/#install-subdirs)
...
### 5.18. Use `BINARY_ALIAS` to Rename Commands Instead of Patching the Build[](https://docs.freebsd.org/en/books/porters-handbook/book/#binary-alias)
...
## Chapter 6. Special Considerations[](https://docs.freebsd.org/en/books/porters-handbook/book/#special)

This section explains the most common things to consider when creating a port.

### 6.1. Splitting long files[](https://docs.freebsd.org/en/books/porters-handbook/book/#splitting-long-files)
...
#### 6.2.1. Handling Symbolic Links[](https://docs.freebsd.org/en/books/porters-handbook/book/#staging-symlink)
...
### 6.3. Bundled Libraries[](https://docs.freebsd.org/en/books/porters-handbook/book/#bundled-libs)

This section explains why bundled dependencies are considered bad and what to do about them.

#### 6.3.1. Why Bundled Libraries Are Bad[](https://docs.freebsd.org/en/books/porters-handbook/book/#bundled-libs-why-bad)
...
#### 6.3.2. What to do About Bundled Libraries[](https://docs.freebsd.org/en/books/porters-handbook/book/#bundled-libs-practices)
...
### 6.4. Shared Libraries[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-shlibs)
...
### 6.5. Ports with Distribution Restrictions or Legal Concerns[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions)
...
#### 6.5.1. `NO_PACKAGE`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-no_package)
...
#### 6.5.2. `NO_CDROM`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-no_cdrom)
...
#### 6.5.3. `NOFETCHFILES`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-nofetchfiles)
...
#### 6.5.4. `RESTRICTED`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-restricted)
...
#### 6.5.5. `RESTRICTED_FILES`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-restricted_files)
...
#### 6.5.6. `LEGAL_TEXT`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-legal_text)
...
#### 6.5.7. /usr/ports/LEGAL and `LEGAL`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-legal)
...
#### 6.5.8. Examples[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-restrictions-examples)
...
### 6.6. Building Mechanisms[](https://docs.freebsd.org/en/books/porters-handbook/book/#building)

#### 6.6.1. Building Ports in Parallel[](https://docs.freebsd.org/en/books/porters-handbook/book/#parallel-builds)
...
#### 6.6.2. `make`, `gmake`, and `imake`[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-make)
...
If the port is an X application that uses imake to create Makefiles from Imakefiles, set `USES= imake`. See the [`USES=imake`](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-imake) section of [Using `USES` Macros](https://docs.freebsd.org/en/books/porters-handbook/book/#uses) for more details.
...
#### 6.6.3. `configure` Script[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-configure)
...
#### 6.6.4. Using `cmake`[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-cmake)
...
| `CMAKE_ON` | For each entry in `CMAKE_ON`, an enabled boolean value is added to `CMAKE_ARGS`. See [`CMAKE_ON` and `CMAKE_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#using-cmake-example2). |
| `CMAKE_OFF` | For each entry in `CMAKE_OFF`, a disabled boolean value is added to `CMAKE_ARGS`. See [`CMAKE_ON` and `CMAKE_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#using-cmake-example2). |
...
|     | This is only for the default values off `CMAKE_ARGS`. The helpers described in [`OPT_CMAKE_BOOL` and `OPT_CMAKE_BOOL_OFF`](https://docs.freebsd.org/en/books/porters-handbook/book/#options-cmake_bool) use the same semantics, but for optional values. |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

#### 6.6.5. Using `scons`[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-scons)
...
#### 6.6.6. Building Rust Applications with `cargo`[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-cargo)
...
```
% make makesum
=> Aaronepower-tokei-v7.0.2_GH0.tar.gz doesn't seem to exist in /usr/ports/distfiles/.=> Attempting to fetch https://codeload.github.com/Aaronepower/tokei/tar.gz/v7.0.2?dummy=/Aaronepower-tokei-v7.0.2_GH0.tar.gzfetch: https://codeload.github.com/Aaronepower/tokei/tar.gz/v7.0.2?dummy=/Aaronepower-tokei-v7.0.2_GH0.tar.gz: size of remote file is not knownAaronepower-tokei-v7.0.2_GH0.tar.gz                     45 kB  239 kBps 00m00s
```
...
```
% make makesum
=> rust/crates/aho-corasick-0.6.4.tar.gz doesn't seem to exist in /usr/ports/distfiles/.=> Attempting to fetch https://crates.io/api/v1/crates/aho-corasick/0.6.4/download?dummy=/rust/crates/aho-corasick-0.6.4.tar.gzrust/crates/aho-corasick-0.6.4.tar.gz         100% of   24 kB 6139 kBps 00m00s=> rust/crates/ansi_term-0.11.0.tar.gz doesn't seem to exist in /usr/ports/distfiles/.
=> Attempting to fetch https://crates.io/api/v1/crates/ansi_term/0.11.0/download?dummy=/rust/crates/ansi_term-0.11.0.tar.gz
rust/crates/ansi_term-0.11.0.tar.gz           100% of   16 kB   21 MBps 00m00s
=> rust/crates/arrayvec-0.4.7.tar.gz doesn't seem to exist in /usr/ports/distfiles/.=> Attempting to fetch https://crates.io/api/v1/crates/arrayvec/0.4.7/download?dummy=/rust/crates/arrayvec-0.4.7.tar.gzrust/crates/arrayvec-0.4.7.tar.gz             100% of   22 kB 3237 kBps 00m00s=> rust/crates/atty-0.2.9.tar.gz doesn't seem to exist in /usr/ports/distfiles/.
...
#### 6.6.7. Using `meson`[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-meson)
...
#### 6.6.8. Building Go Applications[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-go)
...
=> motemen-ghq-v0.12.5_GH0.tar.gz doesn't seem to exist in /usr/ports/distfiles/.=> Attempting to fetch https://codeload.github.com/motemen/ghq/tar.gz/v0.12.5?dummy=/motemen-ghq-v0.12.5_GH0.tar.gzfetch: https://codeload.github.com/motemen/ghq/tar.gz/v0.12.5?dummy=/motemen-ghq-v0.12.5_GH0.tar.gz: size of remote file is not knownmotemen-ghq-v0.12.5_GH0.tar.gz                          32 kB  177 kBps    00s
```
...
```
% make makesum
=> Songmu-gitconfig-v0.0.2_GH0.tar.gz doesn't seem to exist in /usr/ports/distfiles/.=> Attempting to fetch https://codeload.github.com/Songmu/gitconfig/tar.gz/v0.0.2?dummy=/Songmu-gitconfig-v0.0.2_GH0.tar.gzfetch: https://codeload.github.com/Songmu/gitconfig/tar.gz/v0.0.2?dummy=/Songmu-gitconfig-v0.0.2_GH0.tar.gz: size of remote file is not knownSongmu-gitconfig-v0.0.2_GH0.tar.gz                    5662  B  936 kBps    00s=> daviddengcn-go-colortext-186a3d44e920_GH0.tar.gz doesn't seem to exist in /usr/ports/distfiles/.
...
#### 6.6.9. Building Haskell Applications with `cabal`[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-cabal)
...
```
% make makesum
=> ShellCheck-0.6.0.tar.gz doesn't seem to exist in /usr/local/poudriere/ports/git/distfiles/cabal.=> Attempting to fetch https://hackage.haskell.org/package/ShellCheck-0.6.0/ShellCheck-0.6.0.tar.gzShellCheck-0.6.0.tar.gz                                136 kB  642 kBps    00s=> QuickCheck-2.12.6.1/QuickCheck-2.12.6.1.tar.gz doesn't seem to exist in /usr/local/poudriere/ports/git/distfiles/cabal.
...
### 6.7. Using GNU Autotools[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-autotools)
...
### 6.8. Using GNU `gettext`[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-gettext)

#### 6.8.1. Basic Usage[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-gettext-basic)
...
#### 6.8.2. Optional Usage[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-gettext-optional)
...
#### 6.8.3. Handling Message Catalog Directories[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-gettext-catalog-directories)
...
### 6.9. Using Perl[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-perl)
...
### 6.10. Using X11[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-x11)

#### 6.10.1. X.Org Components[](https://docs.freebsd.org/en/books/porters-handbook/book/#x11-variables)
...
#### 6.10.2. Ports That Require Motif[](https://docs.freebsd.org/en/books/porters-handbook/book/#x11-motif)
...
#### 6.10.3. X11 Fonts[](https://docs.freebsd.org/en/books/porters-handbook/book/#x11-fonts)

If the port installs fonts for the X Window System, put them in LOCALBASE/lib/X11/fonts/local.

#### 6.10.4. Getting a Fake `DISPLAY` with Xvfb[](https://docs.freebsd.org/en/books/porters-handbook/book/#x11-fake-display)
...
#### 6.10.5. Desktop Entries[](https://docs.freebsd.org/en/books/porters-handbook/book/#desktop-entries)
...
##### 6.10.5.1. Using Predefined .desktop Files[](https://docs.freebsd.org/en/books/porters-handbook/book/#desktop-entries-predefined)

Ports that include predefined *.desktop must include those files in pkg-plist and install them in the $LOCALBASE/share/applications directory. The [`INSTALL_DATA` macro](https://docs.freebsd.org/en/books/porters-handbook/book/#install-macros) is useful for installing these files.

##### 6.10.5.2. Updating Desktop Database[](https://docs.freebsd.org/en/books/porters-handbook/book/#updating-desktop-database)
...
##### 6.10.5.3. Creating Desktop Entries with `DESKTOP_ENTRIES`[](https://docs.freebsd.org/en/books/porters-handbook/book/#desktop-entries-macro)
...
### 6.11. Using GNOME[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-gnome)

#### 6.11.1. Introduction[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-gnome-introduction)
...
#### 6.11.2. Using `USE_GNOME`[](https://docs.freebsd.org/en/books/porters-handbook/book/#use-gnome)
...
#### 6.11.3. Variables[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-gnome-variables)
...
### 6.12. GNOME Components[](https://docs.freebsd.org/en/books/porters-handbook/book/#gnome-components)
...
### 6.13. Using Qt[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-qt)

|     | For ports that are part of Qt itself, see [`qt-dist`](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-qt-dist). |
| --- | ----------------------------------------------------------------------------------------------------------------------------- |

#### 6.13.1. Ports That Require Qt[](https://docs.freebsd.org/en/books/porters-handbook/book/#qt-common)
...
#### 6.13.2. Component Selection[](https://docs.freebsd.org/en/books/porters-handbook/book/#qt-components)
...
### 6.14. Using KDE[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-kde)

#### 6.14.1. KDE Variable Definitions[](https://docs.freebsd.org/en/books/porters-handbook/book/#kde5-variables)
...
### 6.16. Using Java[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-java)

#### 6.16.1. Variable Definitions[](https://docs.freebsd.org/en/books/porters-handbook/book/#java-variables)
...
#### 6.16.2. Building with Ant[](https://docs.freebsd.org/en/books/porters-handbook/book/#java-building-with-ant)
...
#### 6.16.3. Best Practices[](https://docs.freebsd.org/en/books/porters-handbook/book/#java-best-practices)
...
### 6.17. Web Applications, Apache and PHP[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-php)

#### 6.17.1. Apache[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-apache)

|     |     |
| --- | --- |Table 31. Variables for Ports That Use Apache
| `USE_APACHE` | The port requires Apache. Possible values: `yes` (gets any version), `22`, `24`, `22-24`, `22+`, etc. The default APACHE version is `22`. More details are available in ports/Mk/bsd.apache.mk and at [wiki.freebsd.org/Apache/](https://wiki.freebsd.org/Apache/). |
...
#### 6.17.2. Web Applications[](https://docs.freebsd.org/en/books/porters-handbook/book/#web-apps)
...
#### 6.17.3. PHP[](https://docs.freebsd.org/en/books/porters-handbook/book/#php-variables)
...
#### 6.17.4. PEAR Modules[](https://docs.freebsd.org/en/books/porters-handbook/book/#php-pear)
...
##### 6.17.4.1. Horde Modules[](https://docs.freebsd.org/en/books/porters-handbook/book/#php-horde)
...
### 6.18. Using Python[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-python)
...
| `USE_PYTHON=flavors` | The port does not use distutils but still supports multiple Python versions. `FLAVORS` will be set to the supported Python versions. See [`USES`=python and Flavors](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors-auto-python) for more information. |
...
### 6.19. Using Tcl/Tk[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-tcl)
...
### 6.21. Using wxWidgets[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-wx)

This section describes the status of the wxWidgets libraries in the ports tree and its integration with the ports system.

#### 6.21.1. Introduction[](https://docs.freebsd.org/en/books/porters-handbook/book/#wx-introduction)
...
#### 6.21.2. Version Selection[](https://docs.freebsd.org/en/books/porters-handbook/book/#wx-version)
...
#### 6.21.3. Component Selection[](https://docs.freebsd.org/en/books/porters-handbook/book/#wx-components)
...
#### 6.21.4. Detecting Installed Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#wx-version-detection)
...
#### 6.21.5. Defined Variables[](https://docs.freebsd.org/en/books/porters-handbook/book/#wx-defined-variables)
...
#### 6.21.6. Processing in bsd.port.pre.mk[](https://docs.freebsd.org/en/books/porters-handbook/book/#wx-premk)
...
#### 6.21.7. Additional `configure` Arguments[](https://docs.freebsd.org/en/books/porters-handbook/book/#wx-additional-config-args)
...
### 6.22. Using Lua[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-lua)

This section describes the status of the Lua libraries in the ports tree and its integration with the ports system.

#### 6.22.1. Introduction[](https://docs.freebsd.org/en/books/porters-handbook/book/#lua-introduction)
...
#### 6.22.2. Version Selection[](https://docs.freebsd.org/en/books/porters-handbook/book/#lua-version)
...
#### 6.22.3. Configuration and Compiler flags[](https://docs.freebsd.org/en/books/porters-handbook/book/#lua-version-config)
...
#### 6.22.4. Version Flavors[](https://docs.freebsd.org/en/books/porters-handbook/book/#lua-version-flavors)
...
#### 6.22.5. Defined Variables[](https://docs.freebsd.org/en/books/porters-handbook/book/#lua-defined-variables)
...
#### 6.22.6. Examples[](https://docs.freebsd.org/en/books/porters-handbook/book/#lua-examples)
...
### 6.23. Using Guile[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-guile)

This section describes the status of Guile in the ports tree and its integration with the ports system.

#### 6.23.1. Introduction[](https://docs.freebsd.org/en/books/porters-handbook/book/#guile-introduction)
...
#### 6.23.2. Version Selection[](https://docs.freebsd.org/en/books/porters-handbook/book/#guile-version)
...
| Name | Description |
| --- | --- |
| `*X.Y*` | Declare compatibility with Guile version `X.Y`. Currently available versions are `1.8` (obsolete), `2.2` and `3.0`. Multiple versions may be specified. |
...
#### 6.23.3. Configuration flags[](https://docs.freebsd.org/en/books/porters-handbook/book/#guile-version-config)
...
#### 6.23.4. Version Flavors[](https://docs.freebsd.org/en/books/porters-handbook/book/#guile-version-flavors)
...
#### 6.23.5. Defined Variables[](https://docs.freebsd.org/en/books/porters-handbook/book/#guile-defined-variables)
...
| `GUILE_*_CMD`<br>`GUILE_*_CMDPATH` |     | Like `GUILE_CMD` and `GUILE_CMDPATH`, but for other tool binaries. |
...
#### 6.23.6. Examples[](https://docs.freebsd.org/en/books/porters-handbook/book/#guile-examples)
...
### 6.24. Using `iconv`[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-iconv)
...
### 6.25. Using Xfce[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-xfce)
...
|     | Xfce components included this way will automatically include any dependencies they need. It is no longer necessary to specify the entire list. If the port only needs [x11-wm/xfce4-panel](https://cgit.freebsd.org/ports/tree/x11-wm/xfce4-panel/), use:<br><br>USES=        xfce<br>USE_XFCE=    panel<br><br>There is no need to list the components [x11-wm/xfce4-panel](https://cgit.freebsd.org/ports/tree/x11-wm/xfce4-panel/) needs itself like this:<br><br>USES=        xfce<br>USE_XFCE=    libexo libmenu libutil panel<br><br>However, Xfce components and non-Xfce dependencies of the port must be included explicitly. Do not count on an Xfce component to provide a sub-dependency other than itself for the main port. |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

### 6.26. Using Budgie[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-budgie)
...
|     | All application widgets communicate through the **org.budgie_desktop.Raven** service.<br><br>The default dependency is lib- and run-time, it can be changed with `:build` or `:run`, for example:<br><br>USES=        budgie<br>USE_BUDGIE=    screensaver:build |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
### 6.27. Using Databases[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-databases)

Use one of the `USES` macros from [Database `USES` Macros](https://docs.freebsd.org/en/books/porters-handbook/book/#using-databases-uses) to add a dependency on a database.
...
### 6.28. Starting and Stopping Services (`rc` Scripts)[](https://docs.freebsd.org/en/books/porters-handbook/book/#rc-scripts)
...
#### 6.28.1. Pre-Commit Checklist[](https://docs.freebsd.org/en/books/porters-handbook/book/#rc-scripts-checklist)
...
### 6.29. Adding Users and Groups[](https://docs.freebsd.org/en/books/porters-handbook/book/#users-and-groups)
...
### 6.30. Ports That Rely on Kernel Sources[](https://docs.freebsd.org/en/books/porters-handbook/book/#requiring-kernel-sources)
...
### 6.31. Go Libraries[](https://docs.freebsd.org/en/books/porters-handbook/book/#go-libs)
...
### 6.32. Haskell Libraries[](https://docs.freebsd.org/en/books/porters-handbook/book/#haskell-libs)
...
### 6.33. Shell Completion Files[](https://docs.freebsd.org/en/books/porters-handbook/book/#shell-completion)
...
| `bash` | ${PREFIX}/etc/bash_completion.d or ${PREFIX}/share/bash-completion/completions | (any unique file names in one of these folders) |
...
## Chapter 7. Flavors[](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors)

### 7.1. An Introduction to Flavors[](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors-intro)
...
### 7.2. Using FLAVORS[](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors-using)

To declare a port having multiple flavors, add `FLAVORS` to its Makefile. The first flavor in `FLAVORS` is the default flavor.

|     | It can help simplify the logic of the Makefile to also define `FLAVOR` as:<br><br>FLAVOR?=    ${FLAVORS:[1]} |
| --- | ------------------------------------------------------------------------------------------------------------ |
...
#### 7.2.1. Flavors Helpers[](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors-using-helpers)

To make the Makefile easier to write, a few flavors helpers exist.

This list of helpers will set their variable:

* `*flavor*_PKGNAMEPREFIX`

* `*flavor*_PKGNAMESUFFIX`
...
* `*flavor*_CONFLICTS_BUILD`

* `*flavor*_CONFLICTS_INSTALL`

* `*flavor*_PKG_DEPENDS`

* `*flavor*_EXTRACT_DEPENDS`

* `*flavor*_PATCH_DEPENDS`

* `*flavor*_FETCH_DEPENDS`
...
* `*flavor*_TEST_DEPENDS`

Example 101. Flavor Specific `PKGNAME`

As all packages must have a different package name, flavors must change theirs, using `*flavor*_PKGNAMEPREFIX` and `*flavor*_PKGNAMESUFFIX` makes this easy:

FLAVORS=    normal lite
lite_PKGNAMESUFFIX=    -lite

### 7.3. `USES=php` and Flavors[](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors-auto-php)
...
#### 7.3.1. PHP Flavors with PHP Applications[](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors-auto-php-app)
...
### 7.4. `USES=python` and Flavors[](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors-auto-python)
...
### 7.5. `USES=lua` and Flavors[](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors-auto-lua)
...
### 7.6. `USES=guile` and Flavors[](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors-auto-guile)
...
## Chapter 8. Advanced pkg-plist Practices[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist)

### 8.1. Changing pkg-plist Based on Make Variables[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-sub)

Some ports, particularly the `p5-` ports, need to change their pkg-plist depending on what options they are configured with (or version of `perl`, in the case of `p5-` ports). To make this easy, any instances in pkg-plist of `%%OSREL%%`, `%%PERL_VER%%`, and `%%PERL_VERSION%%` will be substituted appropriately. The value of `%%OSREL%%` is the numeric revision of the operating system (for example, `4.9`). `%%PERL_VERSION%%` and `%%PERL_VER%%` is the full version number of `perl` (for example, `5.8.9`). Several other `%%*VARS*%%` related to port’s documentation files are described in [the relevant section](https://docs.freebsd.org/en/books/porters-handbook/book/#install-documentation).
...
If files are installed conditionally on the options set in the port, the usual way of handling it is prefixing pkg-plist lines with a `%%OPT%%` for lines needed when the option is enabled, or `%%NO_OPT%%` when the option is disabled, and adding `OPTIONS_SUB=yes` to the Makefile. See [`OPTIONS_SUB`](https://docs.freebsd.org/en/books/porters-handbook/book/#options_sub) for more information.
...
### 8.2. Empty Directories[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-cleaning)

#### 8.2.1. Cleaning Up Empty Directories[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-dir-cleaning)
...
#### 8.2.2. Creating Empty Directories[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-dir-empty)
...
### 8.3. Configuration Files[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-config)
...
Instead, install sample files with a filename.sample extension. The `@sample` macro automates this, see [`@sample` *file* [*file*]](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-sample) for what it does exactly. For each sample file, add a line to pkg-plist:
...
|     | The sample configuration files should always have the .sample suffix. If for some historical reason using the standard suffix is not possible, or if the sample files come from some other directory, use this construct:<br><br>@sample etc/orbit.conf-dist etc/orbit.conf<br><br>or<br><br>@sample %%EXAMPLESDIR%%/orbit.conf etc/orbit.conf<br><br>The format is `@sample *sample-file actual-config-file*`. |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

### 8.4. Dynamic Versus Static Package List[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-dynamic)
...
### 8.5. Automated Package List Creation[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-autoplist)
...
#### 8.5.1. Expanding `PLIST_SUB` with Regular Expressions[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-autoplist-regex)
...
### 8.6. Expanding Package List with Keywords[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords)
...
|     | If a keyword is used on an [optional](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-options) entry, it must to be added after the helper:<br><br>%%FOO%%@sample etc/orbit.conf.sample<br><br>This is because the options plist helpers are used to comment out the line, so they need to be put first. See [`OPTIONS_SUB`](https://docs.freebsd.org/en/books/porters-handbook/book/#options_sub) for more information. |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |

#### 8.6.1. `@desktop-file-utils`[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-desktop-file-utils)
...
#### 8.6.2. `@fc` *directory*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-fc)
...
#### 8.6.3. `@fontsdir` *directory*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-fontsdir)
...
#### 8.6.4. `@info` *file*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-info)
...
#### 8.6.5. `@kld` *directory*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-kld)
...
#### 8.6.6. `@rmtry` *file*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-rmtry)

Will remove the file on deinstallation, and not give an error if the file is not there.

#### 8.6.7. `@sample` *file* [*file*][](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-sample)
...
#### 8.6.8. `@shared-mime-info` *directory*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-shared-mime-info)

Runs `update-mime-database` on the directory on installation and deinstallation.

#### 8.6.9. `@shell` *file*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-shell)
...
#### 8.6.10. `@terminfo`[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-terminfo)

Do not use by itself. If the port installs *.terminfo files, add [](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-terminfo)to its Makefile.
...
#### 8.6.11. Base Keywords[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base)
...
##### 8.6.11.1. `@` [*file*][](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-empty)
...
##### 8.6.11.2. `@preexec` *command*, `@postexec` *command*, `@preunexec` *command*, `@postunexec` *command*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-exec)
...
##### 8.6.11.3. `@mode` *mode*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-mode)
...
##### 8.6.11.4. `@owner` *user*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-owner)

Set default ownership for all subsequent files to *user*. Use without an argument to set back to default ownership (`root`).

##### 8.6.11.5. `@group` *group*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-group)
...
##### 8.6.11.6. `@comment` *string*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-comment)

This line is ignored when packing.

##### 8.6.11.7. `@dir` *directory*[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-dir)
...
##### 8.6.11.8. `@exec` *command*, `@unexec` *command* (Deprecated)[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-exec-deprecated)

Execute *command* as part of the installation or deinstallation process. Please use [`@preexec` *command*, `@postexec` *command*, `@preunexec` *command*, `@postunexec` *command*](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-exec) instead.

##### 8.6.11.9. `@dirrm` *directory* (Deprecated)[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-dirrm)
...
##### 8.6.11.10. `@dirrmtry` *directory* (Deprecated)[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-base-dirrmtry)

Declare directory name to be removed, as for `@dirrm`, but does not issue a warning if the directory cannot be removed.

#### 8.6.12. Creating New Keywords[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-creating-new)
...
##### 8.6.12.1. `attributes`[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-attributes)
...
##### 8.6.12.2. `action`[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-action)
...
##### 8.6.12.3. `arguments`[](https://docs.freebsd.org/en/books/porters-handbook/book/#plist-keywords-arguments)
...
## Chapter 9. pkg-*[](https://docs.freebsd.org/en/books/porters-handbook/book/#pkg-files)

There are some tricks we have not mentioned yet about the pkg-* files that come in handy sometimes.

### 9.1. pkg-message[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-message)
...
|     | * pkg-message must contain only information that is *vital* to setup and operation on FreeBSD, and that is unique to the port in question.<br>* Setup information should only be shown on initial install. Upgrade instructions should be shown only when upgrading from the relevant version.<br>* Do not surround the messages with either whitespace or lines of symbols (like `----------`, , or `==========`). Leave the formatting to [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html).<br>* Committers have blanket approval to constrain existing messages to install or upgrade ranges using the UCL format specifications.<br>* Please be sure to refer to the proper tools for handling services.<br>  <br>  * Use `service name start` to start a service rather than using `/usr/local/etc/rc.d/name start`<br>  * Use `sysrc name_enable=YES` to change options in rc.conf |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
If the file starts with “[” then it is considered to be a UCL file. The UCL format is described on [libucl’s GitHub page](https://github.com/vstakhov/libucl).

|     | Do not add an entry for pkg-message in pkg-plist. |
| --- | ------------------------------------------------- |

#### 9.1.1. UCL in pkg-message[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-message-ucl)
...
### 9.2. pkg-install, pkg-pre-install, and pkg-post-install[](https://docs.freebsd.org/en/books/porters-handbook/book/#pkg-install)
...
### 9.3. pkg-deinstall, pkg-pre-deinstall, and pkg-post-deinstall[](https://docs.freebsd.org/en/books/porters-handbook/book/#pkg-deinstall)
...
### 9.4. Changing the Names of pkg-*[](https://docs.freebsd.org/en/books/porters-handbook/book/#pkg-names)

All the names of pkg-* are defined using variables that can be changed in the Makefile if needed. This is especially useful when sharing the same pkg-* files among several ports or when it is necessary to write to one of these files. See [writing to places other than `WRKDIR`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-wrkdir) for why it is a bad idea to write directly into the directory containing the pkg-* files.
...
### 9.5. Making Use of `SUB_FILES` and `SUB_LIST`[](https://docs.freebsd.org/en/books/porters-handbook/book/#using-sub-files)
...
## Chapter 10. Testing the Port[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing)

### 10.1. Running `make describe`[](https://docs.freebsd.org/en/books/porters-handbook/book/#make-describe)
...
### 10.2. Running `make test`[](https://docs.freebsd.org/en/books/porters-handbook/book/#make-test)
...
### 10.3. Portclippy / Portfmt[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-portclippy)
...
### 10.4. Portlint[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-portlint)
...
### 10.5. Port Tools[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-porttools)
...
### 10.6. `PREFIX` and `DESTDIR`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-prefix)
...
### 10.7. poudriere[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere)
...
#### 10.7.1. Installing poudriere[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-installing)
...
#### 10.7.2. Setting Up poudriere[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-setup)
...
#### 10.7.3. Creating poudriere Jails[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-create-jails)
...
|     | To create a poudriere jail for `14.0-CURRENT`:<br><br>```<br># poudriere jail -c -j 14amd64 -v main -a amd64 -m git+https<br>```<br><br>In order to run a `14.0-CURRENT` poudriere jail the host must be running `14.0-CURRENT`. In general, newer kernels can build and run older jails. For instance, a `14.0-CURRENT` kernel can build and run a `12.4-STABLE` if the `COMPAT_FREEBSD12` kernel option was compiled in (on by default in `14.0-CURRENT`GENERIC kernel config). |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
#### 10.7.4. Keeping poudriere Jails Updated[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-maintaining-jails)
...
|     | For jails employing a `git+*` method, it is helpful to add `-J *NumberOfParallelBuildJobs*` to speed up the build by increasing the number of parallel compile jobs used. For example, if the building machine has 6 CPUs, use:<br><br>```<br># poudriere jail -u -J 6 -j JAILNAME<br>``` |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

#### 10.7.5. Setting Up Ports Trees for Use with poudriere[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-ports-tree)
...
#### 10.7.6. Using Manually Managed Ports Trees with poudriere[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-ports-tree-manual)
...
#### 10.7.7. Keeping poudriere Ports Trees Updated[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-ports-tree-updating)
...
#### 10.7.8. Testing Ports[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-testing-ports)
...
#### 10.7.9. Using Sets[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-sets)
...
#### 10.7.10. Providing a Custom make.conf File[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-make-conf)
...
#### 10.7.11. Pruning no Longer Needed Distfiles[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-poudriere-pruning-distfiles)
...
### 10.8. Debugging ports[](https://docs.freebsd.org/en/books/porters-handbook/book/#testing-debugging-ports)
...
## Chapter 11. Upgrading a Port[](https://docs.freebsd.org/en/books/porters-handbook/book/#port-upgrading)
...
### 11.1. Using Git to Make Patches[](https://docs.freebsd.org/en/books/porters-handbook/book/#git-diff)
...
Make sure to check the port using the checklist in [Testing the Port](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-testing) and [Checking the Port with `portlint`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-portlint).
...
### 11.2. UPDATING and MOVED[](https://docs.freebsd.org/en/books/porters-handbook/book/#moved-and-updating-files)

#### 11.2.1. /usr/ports/UPDATING[](https://docs.freebsd.org/en/books/porters-handbook/book/#moved-and-updating-updating)
...
|     | When including exact portmaster, portupgrade, and/or pkg instructions, please make sure to get the shell escaping right. For example, do *not* use:<br><br>```<br># pkg delete -g -f docbook-xml* docbook-sk* docbook[2345]??-* docbook-4*<br>```<br><br>As shown, the command will only work with bourne shells. Instead, use the form shown below, which will work with both bourne shell and c-shell:<br><br>```<br># pkg delete -g -f docbook-xml\* docbook-sk\* docbook\[2345\]\?\?-\* docbook-4\*<br>``` |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
#### 11.2.2. /usr/ports/MOVED[](https://docs.freebsd.org/en/books/porters-handbook/book/#moved-and-updating-moved)
...
|     | Any changes must be validated with `Tools/scripts/MOVEDlint.awk`.<br><br>If using a ports directory other than /usr/ports, use:<br><br>```<br>% cd /home/user/ports<br>% env PORTSDIR=$PWD Tools/scripts/MOVEDlint.awk<br>``` |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

## Chapter 12. Security[](https://docs.freebsd.org/en/books/porters-handbook/book/#security)

### 12.1. Why Security is So Important[](https://docs.freebsd.org/en/books/porters-handbook/book/#security-intro)
...
### 12.3. Keeping the Community Informed[](https://docs.freebsd.org/en/books/porters-handbook/book/#security-notify)

#### 12.3.1. The VuXML Database[](https://docs.freebsd.org/en/books/porters-handbook/book/#security-notify-vuxml-db)
...
#### 12.3.2. A Short Introduction to VuXML[](https://docs.freebsd.org/en/books/porters-handbook/book/#security-notify-vuxml-intro)
...
#### 12.3.3. Testing Changes to the VuXML Database[](https://docs.freebsd.org/en/books/porters-handbook/book/#security-notify-vuxml-testing)
...
## Chapter 13. Dos and Don’ts[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-dads)

### 13.1. Introduction[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-intro)
...
### 13.2. `WRKDIR`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-wrkdir)
...
### 13.3. `WRKDIRPREFIX`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-wrkdirprefix)
...
### 13.4. Differentiating Operating Systems and OS Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-versions)
...
A complete list of `__FreeBSD_version` values is available in [__FreeBSD_version Values](https://docs.freebsd.org/en/books/porters-handbook/book/#versions).

### 13.5. Writing Something After bsd.port.mk[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-after-port-mk)
...
| `OSVERSION` | The numeric version of the operating system; the same as [`__FreeBSD_version`](https://docs.freebsd.org/en/books/porters-handbook/book/#versions). |
| `LOCALBASE` | The base of the "local" tree (for example, `/usr/local`)                                                                                           |
| `PREFIX`    | Where the port installs itself (see [more on `PREFIX`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-prefix)).                  |
...
### 13.6. Use the `exec` Statement in Wrapper Scripts[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-sh-exec)
...
### 13.7. Do Things Rationally[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-rational)
...
### 13.8. Respect Both `CC` and `CXX`[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-cc)
...
### 13.9. Respect `CFLAGS`[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-cflags)
...
### 13.10. Verbose Build Logs[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-verbose-logs)
...
### 13.11. Feedback[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-feedback)
...
### 13.12. README.html[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-readme)
...
### 13.13. Marking a Port Not Installable with `BROKEN`, `FORBIDDEN`, or `IGNORE`[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall)
...
#### 13.13.1. Variables[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall-variables)
...
* `IGNORE` is reserved for ports that must not be built for some other reason. Use it for ports where the problem is believed to be structural. The build cluster will not, under any circumstances, build ports marked as `IGNORE`. For instance, use `IGNORE` when a port:
   ...
   * does not work with some other currently installed port (for instance, the port depends on [www/drupal7](https://cgit.freebsd.org/ports/tree/www/drupal7/) but [www/drupal8](https://cgit.freebsd.org/ports/tree/www/drupal8/) is installed)
     ...
     |     | If a port would conflict with a currently installed port (for example, if they install a file in the same place that performs a different function), [use `CONFLICTS` instead](https://docs.freebsd.org/en/books/porters-handbook/book/#conflicts). `CONFLICTS` will set `IGNORE` by itself. |
     | --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

#### 13.13.2. Implementation Notes[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall-notes)
...
### 13.14. Architectural Considerations[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-arch)

#### 13.14.1. General Notes on Architectures[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-arch-general)
...
#### 13.14.2. Marking a Port as Architecture Neutral[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-arch-neutral)
...
#### 13.14.3. Marking a Port as Ignored Only On Certain Architectures[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-arch-ignore)
...
#### 13.14.4. Cluster-Specific Considerations[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-arch-cluster)
...
### 13.15. Marking a Port for Removal with `DEPRECATED` or `EXPIRATION_DATE`[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-deprecated)
...
### 13.16. Avoid Use of the `.error` Construct[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-dot-error)
...
It is a common mistake to use `.error` for this purpose. The problem with this is that many automated tools that work with the ports tree will fail in this situation. The most common occurrence of this is seen when trying to build /usr/ports/INDEX (see [Running `make describe`](https://docs.freebsd.org/en/books/porters-handbook/book/#make-describe)). However, even more trivial commands such as `make maintainer` also fail in this scenario. This is not acceptable.
...
### 13.17. Usage of sysctl[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-sysctl)
...
### 13.18. Rerolling Distfiles[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-rerolling-distfiles)
...
### 13.19. Use POSIX Standards[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-use-posix-standards)
...
### 13.20. Miscellanea[](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-misc)
...
## Chapter 14. A Sample Makefile[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-samplem)
...
## Chapter 15. Order of Variables in Port Makefiles[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order)
...
### 15.1. `PORTNAME` Block[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-portname)
...
* [`MASTER_SITES`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites)

* [`MASTER_SITE_SUBDIR`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-shorthand) (deprecated)
...
* [`EXTRACT_SUFX`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-extract_sufx)

* [`DISTFILES`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-distfiles-definition)

* [`DIST_SUBDIR`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-dist_subdir)

* [`EXTRACT_ONLY`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-extract_only)

|     | Only one of PORTVERSION and DISTVERSION can be used. |
| --- | ---------------------------------------------------- |

### 15.2. `PATCHFILES` Block[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-patch)

This block is optional. The variables are:

* [`PATCH_SITES`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-patchfiles)

* [`PATCHFILES`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-patchfiles)

* [`PATCH_DIST_STRIP`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-patchfiles)

### 15.3. `MAINTAINER` Block[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-maintainer)
...
### 15.4. `LICENSE` Block[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-license)
...
* [`LICENSE_COMB`](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_comb)

* [`LICENSE_GROUPS`](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_groups) or `LICENSE_GROUPS_NAME`

* [`LICENSE_NAME`](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_name) or `LICENSE_NAME_NAME`

* [`LICENSE_TEXT`](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_text) or `LICENSE_TEXT_NAME`

* [`LICENSE_FILE`](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_file) or `LICENSE_FILE_NAME`

* [`LICENSE_PERMS`](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_perms) or `LICENSE_PERMS_NAME_`

* [`LICENSE_DISTFILES`](https://docs.freebsd.org/en/books/porters-handbook/book/#licenses-license_distfiles) or `LICENSE_DISTFILES_NAME`

If there are multiple licenses, sort the different LICENSE_VAR_NAME variables by license name.

### 15.5. Generic `BROKEN`/`IGNORE`/`DEPRECATED` Messages[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-broken)

This block is optional. The variables are:

* [`DEPRECATED`](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-deprecated)

* [`EXPIRATION_DATE`](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-deprecated)
...
* [`BROKEN_*`](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall)

* [`IGNORE`](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall)

* [`IGNORE_*`](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall)

* [`ONLY_FOR_ARCHS`](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall)

* [`ONLY_FOR_ARCHS_REASON*`](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall)

* [`NOT_FOR_ARCHS`](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall)

* [`NOT_FOR_ARCHS_REASON*`](https://docs.freebsd.org/en/books/porters-handbook/book/#dads-noinstall)

|     | `BROKEN_*` and `IGNORE_*` can be any generic variables, for example, `IGNORE_amd64`, `BROKEN_FreeBSD_10`, etc. With the exception of variables that depend on a [`USES`](https://docs.freebsd.org/en/books/porters-handbook/book/#uses), place those in [`USES` and `USE_x`](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-uses). For instance, `IGNORE_WITH_PHP` only works if [`php`](https://docs.freebsd.org/en/books/porters-handbook/book/#xuses-php) is set, and `BROKEN_SSL` only if [`ssl`](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-ssl) is set.<br><br>If the port is marked BROKEN when some conditions are met, and such conditions can only be tested after including bsd.port.options.mk or bsd.port.pre.mk, then those variables should be set later, in [The Rest of the Variables](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-rest). |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

### 15.6. The Dependencies Block[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-depends)

This block is optional. The variables are:

* [`FETCH_DEPENDS`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-fetch_depends)

* [`EXTRACT_DEPENDS`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-extract_depends)

* [`PATCH_DEPENDS`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-patch_depends)

* [`BUILD_DEPENDS`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-build_depends)

* [`LIB_DEPENDS`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-lib_depends)

* [`RUN_DEPENDS`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-run_depends)

* `TEST_DEPENDS`

### 15.7. Flavors[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-flavors)
...
### 15.8. `USES` and `USE_x`[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-uses)

Start this section with defining `USES`, and then possible `USE_x`.

Keep related variables close together. For example, if using [`USE_GITHUB`](https://docs.freebsd.org/en/books/porters-handbook/book/#makefile-master_sites-github), always put the `GH_*` variables right after it.

### 15.9. Standard bsd.port.mk Variables[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-variables)
...
### 15.10. Options and Helpers[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-options)
...
### 15.11. The Rest of the Variables[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-rest)

And then, the rest of the variables that are not mentioned in the previous blocks.

### 15.12. The Targets[](https://docs.freebsd.org/en/books/porters-handbook/book/#porting-order-targets)

After all the variables are defined, the optional [make(1)](https://man.freebsd.org/cgi/man.cgi?query=make&sektion=1&format=html) targets can be defined. Keep `pre-` **before `post-`** and in the same order as the different stages run:
...
|     | When using options helpers target keep them alphabetically sorted, but keep the `**-on**` **before the** `-off`. When also using the main target, keep the main target before the optional ones:<br><br>post-install:<br>    # install generic bits<br>post-install-DOCS-on:<br>    # Install documentation<br>post-install-X11-on:<br>    # Install X11 related bits<br>post-install-X11-off:<br>    # Install bits that should be there if X11 is disabled |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |

## Chapter 16. Keeping Up[](https://docs.freebsd.org/en/books/porters-handbook/book/#keeping-up)

The FreeBSD Ports Collection is constantly changing. Here is some information on how to keep up.

### 16.1. FreshPorts[](https://docs.freebsd.org/en/books/porters-handbook/book/#freshports)
...
### 16.2. The Web Interface to the Source Repository[](https://docs.freebsd.org/en/books/porters-handbook/book/#cgit)
...
### 16.3. The FreeBSD Ports Mailing List[](https://docs.freebsd.org/en/books/porters-handbook/book/#ports-mailing-list)
...
### 16.4. The FreeBSD Port Building Cluster[](https://docs.freebsd.org/en/books/porters-handbook/book/#build-cluster)
...
### 16.5. Portscout: the FreeBSD Ports Distfile Scanner[](https://docs.freebsd.org/en/books/porters-handbook/book/#distfile-survey)
...
## Chapter 17. Using `USES` Macros[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses)

### 17.1. An Introduction to `USES`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-intro)
...
### 17.2. `7z`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-7z)
...
### 17.3. `ada`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-ada)
...
### 17.4. `autoreconf`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-autoreconf)
...
### 17.5. `blaslapack`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-blaslapack)

Possible arguments: (none), `atlas`, `netlib` (default), `gotoblas`, `openblas`

Adds dependencies on Blas / Lapack libraries.

### 17.6. `bdb`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-bdb)
...
The name of the Berkeley DB library. For example, when using [databases/db5](https://cgit.freebsd.org/ports/tree/databases/db5/), it contains `db-5.3`.

`BDB_LIB_CXX_NAME`

The name of the Berkeley DBC++ library. For example, when using [databases/db5](https://cgit.freebsd.org/ports/tree/databases/db5/), it contains `db_cxx-5.3`.

`BDB_INCLUDE_DIR`

The location of the Berkeley DB include directory. For example, when using [databases/db5](https://cgit.freebsd.org/ports/tree/databases/db5/), it will contain `${LOCALBASE}/include/db5`.

`BDB_LIB_DIR`

The location of the Berkeley DB library directory. For example, when using [databases/db5](https://cgit.freebsd.org/ports/tree/databases/db5/), it contains `${LOCALBASE}/lib`.
...
### 17.7. `bison`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-bison)
...
### 17.8. `budgie`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-budgie)
...
### 17.9. `cabal`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-cabal)
...
If the software uses Haskell dependencies, list them in this variable. Each item should be present on Hackage and be listed in form `packagename-*0.1.2*`. Dependencies can have revisions too, which are specified after the `_` symbol. Automatic generation of the dependency list is supported, see [Building Haskell Applications with `cabal`](https://docs.freebsd.org/en/books/porters-handbook/book/#using-cabal).
...
### 17.10. `cargo`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-cargo)

Possible arguments: (none)

Uses Cargo for configuring, building, and testing. It can be used to port Rust applications that use the Cargo build system. For more information see [Building Rust Applications with `cargo`](https://docs.freebsd.org/en/books/porters-handbook/book/#using-cargo).

### 17.11. `charsetfix`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-charsetfix)
...
### 17.12. `cmake`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-cmake)
...
For more information see [Using `cmake`](https://docs.freebsd.org/en/books/porters-handbook/book/#using-cmake).

### 17.13. `compiler`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-compiler)
...
### 17.14. `cpe`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-cpe)
...
### 17.15. `cran`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-cran)
...
### 17.16. `desktop-file-utils`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-desktop-file-utils)
...
### 17.17. `desthack`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-desthack)

Possible arguments: (none)

Changes the behavior of GNU configure to properly support `DESTDIR` in case the original software does not.

### 17.18. `display`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-display)
...
### 17.19. `dos2unix`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-dos2unix)
...
### 17.20. `drupal`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-drupal)
...
### 17.21. `ebur128`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-ebur128)
...
### 17.22. `eigen`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-eigen)

Possible arguments: 2, 3, build (default), run

Add dependency on [math/eigen](https://cgit.freebsd.org/ports/tree/math/eigen/).

### 17.23. `elfctl`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-elfctl)
...
### 17.24. `erlang`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-erlang)
...
### 17.25. `fakeroot`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-fakeroot)

Possible arguments: (none)

Changes some default behavior of build systems to allow installing as a user. See [FakeRoot - Debian Wiki](https://wiki.debian.org/FakeRoot) for more information on `fakeroot`.

### 17.26. `fam`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-fam)
...
### 17.31. `gem`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-gem)
...
### 17.32. `gettext`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-gettext)
...
### 17.33. `gettext-runtime`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-gettext-runtime)
...
### 17.34. `gettext-tools`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-gettext-tools)
...
### 17.35. `ghostscript`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-ghostscript)
...
### 17.36. `gl`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-gl)
...
add a library dependency on libEGL.so from [graphics/libglvnd](https://cgit.freebsd.org/ports/tree/graphics/libglvnd/)

`gbm`

Add a library dependency on libgbm.so from [graphics/mesa-libs](https://cgit.freebsd.org/ports/tree/graphics/mesa-libs/)

`gl`

Add a library dependency on libGL.so from [graphics/libglvnd](https://cgit.freebsd.org/ports/tree/graphics/libglvnd/)

`glesv2`

Add a library dependency on libGLESv2.so from [graphics/libglvnd](https://cgit.freebsd.org/ports/tree/graphics/libglvnd/)

`glew`

Add a library dependency on libGLEW.so from [graphics/glew](https://cgit.freebsd.org/ports/tree/graphics/glew/)

`glu`

Add a library dependency on libGLU.so from [graphics/libGLU](https://cgit.freebsd.org/ports/tree/graphics/libGLU/)

`glut`

Add a library dependency on libglut.so from [graphics/freeglut](https://cgit.freebsd.org/ports/tree/graphics/freeglut/)

`opengl`

Add a library dependency on libOpenGL.so from [graphics/libglvnd](https://cgit.freebsd.org/ports/tree/graphics/libglvnd/)

### 17.37. `gmake`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-gmake)
...
### 17.38. `gnome`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-gnome)
...
### 17.39. `go`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-go)
...
### 17.40. `gperf`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-gperf)
...
### 17.41. `grantlee`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-grantlee)
...
### 17.42. `groff`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-groff)
...
### 17.43. `gssapi`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-gssapi)
...
### 17.44. `gstreamer`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-gstreamer)
...
### 17.45. `guile`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-guile)

Possible arguments: (none), `*X.Y*`, `flavors`, `build`, `run`, `alias`, `conflicts`

Adds a dependency on Guile. By default this is a library dependency on the appropriate `libguile*.so`, unless overridden by the `build` and/or `run` option. The `alias` option configures `BINARY_ALIAS` appropriately (see [Use `BINARY_ALIAS`](https://docs.freebsd.org/en/books/porters-handbook/book/#binary-alias)).
...
### 17.46. `horde`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-horde)
...
### 17.49. `kde`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-kde)
...
### 17.50. `kmod`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-kmod)
...
### 17.51. `ldap`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-ldap)
...
### 17.52. `lha`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-lha)

Possible arguments: (none)

Set `EXTRACT_SUFX` to `.lzh`

### 17.53. `libarchive`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-libarchive)
...
### 17.54. `libedit`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-libedit)
...
### 17.55. `libtool`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-libtool)
...
### 17.56. `linux`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-linux)
...
### 17.57. `llvm`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-llvm)
...
### 17.58. `localbase`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-localbase)
...
### 17.59. `lua`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-lua)
...
### 17.60. `luajit`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-luajit)
...
### 17.61. `lxqt`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-lxqt)
...
### 17.62. `magick`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-magick)
...
### 17.63. `makeinfo`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-makeinfo)

Possible arguments: (none)

Add a build-time dependency on `makeinfo` if it is not present in the base system.

### 17.64. `makeself`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-makeself)

Possible arguments: (none)

Indicates that the distribution files are makeself archives and sets the appropriate dependencies.

### 17.65. `mate`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-mate)
...
### 17.66. `meson`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-meson)

Possible arguments: (none)

Provide support for Meson based projects. For more information see [Using `meson`](https://docs.freebsd.org/en/books/porters-handbook/book/#using-meson).

### 17.67. `metaport`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-metaport)
...
### 17.68. `minizip`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-minizip)
...
### 17.69. `mysql`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-mysql)
...
### 17.70. `mono`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-mono)
...
### 17.71. `motif`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-motif)
...
### 17.72. `ncurses`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-ncurses)

Possible arguments: (none), `base`, `port`

Uses ncurses, and causes some useful variables to be set.

### 17.73. `nextcloud`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-nextcloud)
...
### 17.74. `ninja`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-ninja)
...
### 17.75. `nodejs`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-nodejs)
...
### 17.76. `objc`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-objc)

Possible arguments: (none)

Add objective C dependencies (compiler, runtime library) if the base system does not support it.

### 17.77. `octave`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-octave)
...
### 17.78. `openal`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-openal)
...
### 17.79. `pathfix`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-pathfix)
...
### 17.80. `pear`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-pear)
...
### 17.81. `perl5`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-perl5)
...
### 17.82. `pgsql`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-pgsql)
...
### 17.83. `php`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-php)
...
Enable automatic [PHP flavors](https://docs.freebsd.org/en/books/porters-handbook/book/#flavors-auto-php) generation. Flavors will be generated for all PHP versions, except the ones present in [`IGNORE_WITH_PHP`](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-php-ignore).
...
### 17.84. `pkgconfig`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-pkgconfig)
...
### 17.85. `pure`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-pure)
...
### 17.86. `pyqt`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-pyqt)
...
### 17.87. `pytest`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-pytest)
...
### 17.88. `python`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-python)
...
### 17.89. `qmail`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-qmail)
...
### 17.90. `qmake`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-qmake)

Possible arguments: (none), `norecursive`, `outsource`, `no_env`, `no_configure`

Uses QMake for configuring. For more information see [Using `qmake`](https://docs.freebsd.org/en/books/porters-handbook/book/#using-qmake).

### 17.91. `qt`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-qt)
...
### 17.92. `qt-dist`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-qt-dist)
...
### 17.93. `readline`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-readline)
...
### 17.94. `ruby`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-ruby)
...
### 17.95. `samba`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-samba)
...
### 17.96. `scons`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-scons)

Possible arguments: (none)

Provide support for the use of [devel/scons](https://cgit.freebsd.org/ports/tree/devel/scons/). See [Using `scons`](https://docs.freebsd.org/en/books/porters-handbook/book/#using-scons) for more information.

### 17.97. `shared-mime-info`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-shared-mime-info)
...
### 17.98. `shebangfix`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-shebangfix)
...
Contains *one* extended regular expressions, and is used with the `-iregex` argument of [find(1)](https://man.freebsd.org/cgi/man.cgi?query=find&sektion=1&format=html). See [`USES=shebangfix` with `SHEBANG_REGEX`](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-shebangfix-ex-regex).

`SHEBANG_GLOB`

Contains a list of patterns used with the `-name` argument of [find(1)](https://man.freebsd.org/cgi/man.cgi?query=find&sektion=1&format=html). See [`USES=shebangfix` with `SHEBANG_GLOB`](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-shebangfix-ex-glob).

`SHEBANG_FILES`

Contains a list of files or [sh(1)](https://man.freebsd.org/cgi/man.cgi?query=sh&sektion=1&format=html) globs. The shebangfix macro is run from `${WRKSRC}`, so `SHEBANG_FILES` can contain paths that are relative to `${WRKSRC}`. It can also deal with absolute paths if files outside of `${WRKSRC}` require patching. See [`USES=shebangfix` with `SHEBANG_FILES`](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-shebangfix-ex-files).
...
|     | `_interp__OLD_CMD` contain multiple values. Any entry with spaces must be quoted. See [Specifying all the Paths When Adding an Interpreter to `USES=shebangfix`](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-shebangfix-ex-ksh). |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
If it was not already defined, and there were no default values for `_interp*OLD_CMD*` *and `_interp`*`CMD` the Ksh entry could be defined as:
...
|     | `SHEBANG_REGEX` is used by running `find -E`, which uses modern regular expressions also known as extended regular expressions. See [re_format(7)](https://man.freebsd.org/cgi/man.cgi?query=re_format&sektion=7&format=html) for more information. |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
### 17.99. `sqlite`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-sqlite)
...
### 17.100. `ssl`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-ssl)
...
|     | If a port does not build with an OpenSSL flavor, set the `BROKEN_SSL` variable, and possibly the `BROKEN_SSL_REASON__flavor_`:<br><br>BROKEN_SSL=    libressl<br>BROKEN_SSL_REASON_libressl=    needs features only available in OpenSSL |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

### 17.101. `tar`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-tar)
...
### 17.102. `tcl`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-tcl)
...
### 17.103. `terminfo`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-terminfo)
...
### 17.104. `tex`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-tex)
...
### 17.105. `tk`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-tk)

Same as arguments for `tcl`

Small wrapper when using both Tcl and Tk. The same variables are returned as when using Tcl.

### 17.106. `uidfix`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-uidfix)
...
### 17.107. `uniquefiles`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-uniquefiles)
...
### 17.108. `vala`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-vala)
...
### 17.109. `varnish`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-varnish)
...
### 17.110. `webplugin`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-webplugin)
...
### 17.111. `xfce`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-xfce)
...
### 17.112. `xorg`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-xorg)
...
### 17.113. `xorg-cat`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-xorg-cat)
...
### 17.114. `zip`[](https://docs.freebsd.org/en/books/porters-handbook/book/#uses-zip)
...
## Chapter 18. `__FreeBSD_version` Values[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions)
...
### 18.1. FreeBSD 15 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-15)

|     |     |     |     |
| --- | --- | --- | --- |Table 53. FreeBSD 15 `__FreeBSD_version` Values
...
### 18.2. FreeBSD 14 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-14)

|     |     |     |     |
| --- | --- | --- | --- |Table 54. FreeBSD 14 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
| 1400043 | [7e1d3eefd410](https://cgit.freebsd.org/src/commit/?id=7e1d3eefd410ca0fbae5a217422821244c3eeee4) | November 25, 2021 | 14.0-CURRENT after removing the unused thread argument from [NDINIT(9)](https://man.freebsd.org/cgi/man.cgi?query=NDINIT&sektion=9&format=html)*. |
| 1400044 | [ec434c85b46d](https://cgit.freebsd.org/src/commit/?id=ec434c85b46dd715da1940e2a8911bf476b0e477) | December 9, 2021 | 14.0-CURRENT after changing in-kernel software crypto ciphers transforms to support AEAD ciphers and changing the Blake-2S/B auth transforms to support Init before Setkey like other auth transforms. |
| 1400045 | [b214fcceacad](https://cgit.freebsd.org/src/commit/?id=b214fcceacad6b842545150664bd2695c1c2b34f) | December 15, 2021 | 14.0-CURRENT after changing [VOP_READDIR(9)](https://man.freebsd.org/cgi/man.cgi?query=VOP_READDIR&sektion=9&format=html)'s cookies argument to a `**uint64_t`. |
| 1400046 | [e2650af157bc](https://cgit.freebsd.org/src/commit/?id=e2650af157bc7489deaf2c9054995f0f88a6e5da) | December 30, 2021 | 14.0-CURRENT after making the CPU_SET macros compatible with glibc. |
| 1400047 | [ed6417cd8d0b](https://cgit.freebsd.org/src/commit/?id=ed6417cd8d0bb5a2c175fce9d8e4a495fae9e9f4) | January 17, 2022 | 14.0-CURRENT after multiple LinuxKPI changes required by drm-kmod. |
| 1400048 | [dd2f7a4b45eb](https://cgit.freebsd.org/src/commit/?id=dd2f7a4b45eb1285e710cfce60cb77f7c11f8075) | January 18, 2022 | 14.0-CURRENT after adding <crypto/chacha20_poly1305.h>. |
| 1400049 | [2c4b65cc3d22](https://cgit.freebsd.org/src/commit/?id=2c4b65cc3d227f31864e183c15f6c42e2c596cd9) | January 24, 2022 | 14.0-CURRENT after adding <crypto/curve25519.h>. |
| 1400050 | [213e91399b79](https://cgit.freebsd.org/src/commit/?id=213e91399b7998554d787bb290109ebe602aa279) | January 25, 2022 | 14.0-CURRENT after iflib adds the feature that a driver can set its own TX queue selection function as `ift_txq_select` in struct `if_txrx`. |
| 1400051 | [59d465e200bb](https://cgit.freebsd.org/src/commit/?id=59d465e200bb7058dfdb183c061730c10dd5bc03) | January 25, 2022 | 14.0-CURRENT after adding i2c support for LinuxKPI. |
| 1400052 | [05f0b24bfb34](https://cgit.freebsd.org/src/commit/?id=05f0b24bfb3416606c8ea02bc1bdb9bcee7aee0c) | February 14, 2022 | 14.0-CURRENT after adding GUID_INIT and pm_qos.h support for LinuxKPI. |
| 1400053 | [ba87e9bf7420](https://cgit.freebsd.org/src/commit/?id=ba87e9bf74202b08b8e3b0a297b9b88f6869fbfb) | February 17, 2022 | 14.0-CURRENT after adding mmap_lock.h to LinuxKPI. |
| 1400054 | [50bb3a33d879](https://cgit.freebsd.org/src/commit/?id=50bb3a33d879536e86e8a23365f070ef00b5cb32) | March 28, 2022 | 14.0-CURRENT after changing `irq_work_queue` to return a bool in LinuxKPI to match 5.10 API. |
| 1400055 | [d69af4758be9](https://cgit.freebsd.org/src/commit/?id=d69af4758be912625ec08656ba64eb90a98c9a7f) | March 29, 2022 | 14.0-CURRENT after adding `for_each_sgtable_dma_sg` and `for_each_sgtable_dma_page` to LinuxKPI |
| 1400056 | [ab8ac4c28574](https://cgit.freebsd.org/src/commit/?id=ab8ac4c28574a42a2891b2e2341f802949c1fb57) | March 31, 2022 | 14.0-CURRENT after zlib upgrade to 1.2.12 |
| 1400057 | [e68b35e40881](https://cgit.freebsd.org/src/commit/?id=e68b35e40881a1bd858e1b4b5003123a484fd7cd) | April 22, 2022 | 14.0-CURRENT after changing udp_tun_func_t() prototype. |
| 1400058 | [2e32d4e41d20](https://cgit.freebsd.org/src/commit/?id=2e32d4e41d205d6f14834f87306a77ff77b9c0bd) | May 7, 2022 | 14.0-CURRENT after newbus changes to remove devclass arguments. |
| 1400059 | [3a9a9c0ca44e](https://cgit.freebsd.org/src/commit/?id=3a9a9c0ca44ec535dcf73fe8462bee458e54814b) | May 14, 2022 | 14.0-CURRENT after upgrading llvm, clang, compiler-rt, libc++, libunwind, lld, lldb and openmp to llvmorg-14.0.3-0-g1f9140064dfb, a.k.a. 14.0.3 release. |
| 1400060 | [85d7875d4291](https://cgit.freebsd.org/src/commit/?id=85d7875d42913c2cb10a007a1be05b210dc6aab2) | June 6, 2022 | 14.0-CURRENT after LinuxKPI dmi_matches() fixes. |
| 1400061 | [c4c5981c14d5](https://cgit.freebsd.org/src/commit/?id=c4c5981c14d5bd69e9df9ae691069ec4c2e92174) | June 8, 2022 | 14.0-CURRENT after mbuf(9) structure changes. |
| 1400062 | [8c309d48aabf](https://cgit.freebsd.org/src/commit/?id=8c309d48aabf1cb469334c7716033f177a2715c0) | June 18, 2022 | 14.0-CURRENT after struct `kinfo_file` changes. |
| 1400063 | [8cff8e6e13a6](https://cgit.freebsd.org/src/commit/?id=8cff8e6e13a6d3ccff40fc0d8d97f5aef22a8f4d) | June 29, 2022 | 14.0-CURRENT after multiple LinuxKPI changes required by drm-kmod. |
| 1400064 | [ddd9004e7a5d](https://cgit.freebsd.org/src/commit/?id=ddd9004e7a5dbf02c34ef0effcef90f7d5df357d) | July 18, 2022 | 14.0-CURRENT after the removal of OBJT_DEFAULT. |
| 1400065 | [b273f93657cf](https://cgit.freebsd.org/src/commit/?id=b273f93657cf0e6f2c6ee4d0f40a43656233c6d0) | August 8, 2022 | 14.0-CURRENT after multiple LinuxKPI changes required by drm-kmod. |
| 1400066 | [ff7812ee7d44](https://cgit.freebsd.org/src/commit/?id=ff7812ee7d444b738a454064f9639c3feb5743e8) | August 18, 2022 | 14.0-CURRENT after multiple LinuxKPI changes required by drm-kmod. |
| 1400069 | [f95c0bc89ea4](https://cgit.freebsd.org/src/commit/?id=f95c0bc89ea4fcde04b0990d57e842ef387ea896) | September 22, 2022 | 14.0-CURRENT after multiple LinuxKPI changes. |
| 1400070 | [6bddde307e21](https://cgit.freebsd.org/src/commit/?id=6bddde307e21eba297ac3f3e534b4cf3be81dfe2) | September 22, 2022 | 14.0-CURRENT after KPI changes to pmap_unmapdev() and kmem_*(). |
...
| 1400072 | [8a96874eeeee](https://cgit.freebsd.org/src/commit/?id=8a96874eeeee5195b0b0952b77227bef6a26d1a6) | September 22, 2022 | 14.0-CURRENT after `qsort_r` prototype modified to match POSIX. |
...
| 1400086 | [c17eb99a66e7](https://cgit.freebsd.org/src/commit/?id=c17eb99a66e7d1e8a64458a969eeb7649f827493) | April 8, 2023 | 14.0-CURRENT after vn_lock_pair() argument changes. |
...
| 1400090 | [653738e895ba](https://cgit.freebsd.org/src/commit/?id=653738e895ba022be1179a95a85089e7bc66dbbe) | June 7, 2023 | 14.0-CURRENT after ptrace started clearing TDB_BORN during PT_DETACH. |
...
| 1400093 | [ba8cc6d7271a](https://cgit.freebsd.org/src/commit/?id=ba8cc6d7271a50fec978a1d3a088aec7985fae48) | July 5, 2023 | 14.0-CURRENT after using __enum_uint8 for vtype and vstate in VFS |
...
### 18.3. FreeBSD 13 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-13)

|     |     |     |     |
| --- | --- | --- | --- |Table 55. FreeBSD 13 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
| 1300026 | [347596](https://svnweb.freebsd.org/changeset/base/347596) | May 14, 2019 | 13.0-CURRENT after adding context member to ww_mutex in LinuxKPI. |
...
| 1300032 | [349151](https://svnweb.freebsd.org/changeset/base/349151) | June 17, 2019 | 13.0-CURRENT after separating kernel `crc32()` implementation to its own header (gsb_crc32.h) and renaming the source to gsb_crc32.c. |
...
| 1300048 | [352700](https://svnweb.freebsd.org/changeset/base/352700) | September 25, 2019 | 13.0-CURRENT after adding a shm_open2 syscall to support the upcoming [memfd_create(2)](https://man.freebsd.org/cgi/man.cgi?query=memfd_create&sektion=2&format=html) syscall. |
...
| 1300062 | [355309](https://svnweb.freebsd.org/changeset/base/355309) | December 2, 2019 | 13.0-CURRENT after adding TCP_STATS support. |
| 1300063 | [355537](https://svnweb.freebsd.org/changeset/base/355537) | December 8, 2019 | 13.0-CURRENT after removal of VI_DOOMED (use VN_IS_DOOMED instead). |
...
| 1300070 | [356113](https://svnweb.freebsd.org/changeset/base/356113) | December 27, 2019 | 13.0-CURRENT after eliminating the last MI difference in AT_* definitions (for powerpc). |
...
| 1300103 | [363757](https://svnweb.freebsd.org/changeset/base/363757) | August 1, 2020 | 13.0-CURRENT after making rights mandatory for NDINIT_ALL. |
...
| 1300113 | [364753](https://svnweb.freebsd.org/changeset/base/364753) | August 25, 2020 | 13.0-CURRENT after adding atomic and `bswap` functions to libcompiler_rt. |
| 1300114 | [365459](https://svnweb.freebsd.org/changeset/base/365459) | September 8, 2020 | 13.0-CURRENT after changing arm64 AT_HWCAP definitions for [elf_aux_info(3)](https://man.freebsd.org/cgi/man.cgi?query=elf_aux_info&sektion=3&format=html). |
...
| 1300524 | [dc4114875ef1](https://cgit.freebsd.org/src/commit/?id=dc4114875ef10618002d3eeb46f09dc42da56b30) | January 14, 2022 | 13.0-STABLE after making the CPU_SET macros compatible with glibc. |
...
| 1301503 | [b2aa64d05bd8](https://cgit.freebsd.org/src/commit/?id=b2aa64d05bd8b04a1bdb63f2a5f9de39c600b463) | May 19, 2022 | 13.1-STABLE after adding alternate DRIVER_MODULE macros without a devclass argument. |
...
| 1301506 | [8e6cfc632cf6](https://cgit.freebsd.org/src/commit/?id=8e6cfc632cf6f9fc906df9d825649443939b55c6) | July 13, 2022 | 13.1-STABLE after adding <crypto/chacha20_poly1305.h> and <crypto/curve25519.h>. |
...
| 1301510 | [6820a0512fa6](https://cgit.freebsd.org/src/commit/?id=6820a0512fa6616ee1da46cb0075da80478776f0) | December 8, 2022 | 13.1-STABLE after LinuxKPI dmi_matches() fixes. |
...
| 1302508 | [21ccba43f511](https://cgit.freebsd.org/src/commit/?id=21ccba43f511cc4089899619b5a85e6d83a200dc) | September 6, 2023 | 13.2-STABLE after ptrace started clearing TDB_BORN during PT_DETACH. |
...
### 18.4. FreeBSD 12 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-12)

|     |     |     |     |
| --- | --- | --- | --- |Table 56. FreeBSD 12 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
| 1200006 | [305256](https://svnweb.freebsd.org/changeset/base/305256) | September 1, 2016 | 12.0-CURRENT after [crunchgen(1)](https://man.freebsd.org/cgi/man.cgi?query=crunchgen&sektion=1&format=html) META_MODE fix in [305254](https://svnweb.freebsd.org/changeset/base/305254). |
...
| 1200025 | [315430](https://svnweb.freebsd.org/changeset/base/315430) | March 16, 2017 | 12.0-CURRENT after addition of the <dev/mmc/mmc_ioctl.h> header. |
...
| 1200043 | [322989](https://svnweb.freebsd.org/changeset/base/322989) | August 28, 2017 | 12.0-CURRENT after dropping x86 CACHE_LINE_SIZE down to 64 bytes. |
...
| 1200047 | [324053](https://svnweb.freebsd.org/changeset/base/324053) | September 26, 2017 | 12.0-CURRENT after removing M_HASHTYPE_RSS_UDP_IPV4_EX. ([324052](https://svnweb.freebsd.org/changeset/base/324052)). |
...
| 1200056 | [327952](https://svnweb.freebsd.org/changeset/base/327952) | January 14, 2018 | 12.0-CURRENT after upgrading clang, llvm, lld, lldb, compiler-rt and libc++ to 6.0.0 (branches/release_60 r321788). |
...
| 1200064 | [334074](https://svnweb.freebsd.org/changeset/base/334074) | May 22, 2018 | 12.0-CURRENT after changing the ifnet address and multicast address TAILQ to CK_STAILQ. |
| 1200065 | [334290](https://svnweb.freebsd.org/changeset/base/334290) | May 28, 2018 | 12.0-CURRENT after changing [dwatch(1)](https://man.freebsd.org/cgi/man.cgi?query=dwatch&sektion=1&format=html) to allow '-E code' to override profile EVENT_DETAILS. |
...
| 1200067 | [334892](https://svnweb.freebsd.org/changeset/base/334892) | June 9, 2018 | 12.0-CURRENT after adding DW_LANG constants to libdwarf. |
...
| 1200075 | [336538](https://svnweb.freebsd.org/changeset/base/336538) | July 19, 2018 | 12.0-CURRENT after `zfsloader` being folded into loader, and after adding ntpd:ntpd as uid:gid 123:123, and after removing arm big-endian support (MACHINE_ARCH=armeb). |
...
| 1200512 | [348245](https://svnweb.freebsd.org/changeset/base/348245) | May 24, 2019 | 12-STABLE after adding context member to ww_mutex in LinuxKPI. |
...
| 1201525 | [365471](https://svnweb.freebsd.org/changeset/base/365471) | September 8, 2020 | 12-STABLE after adding atomic and `bswap` functions to libcompiler_rt. |
...
| 1202501 | [365661](https://svnweb.freebsd.org/changeset/base/365661) | September 12, 2020 | 12-STABLE after followup commits to libcompiler_rt. |
...
| 1203501 | [b148c7b87148](https://cgit.freebsd.org/src/commit/?id=b148c7b87148b653fdbef9c5aa591b9abcd99e26) | December 22, 2021 | 12-STABLE after adding atomic and `bswap` functions to libcompiler_rt. |
...
| 1203504 | [1a398266112e](https://cgit.freebsd.org/src/commit/?id=1a398266112e73f91a4f2e2701ceefd3f2948aac) | December 25, 2021 | 12-STABLE after adding out-of-line LSE atomics helpers to libcompiler_rt.a on aarch64. |
...
### 18.5. FreeBSD 11 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-11)

|     |     |     |     |
| --- | --- | --- | --- |Table 57. FreeBSD 11 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
| 1100048 | [275358](https://svnweb.freebsd.org/changeset/base/275358) | December 1, 2014 | 11.0-CURRENT after starting the process of removing the use of the deprecated "M_FLOWID" flag from the network code. |
...
| 1100090 | [290715](https://svnweb.freebsd.org/changeset/base/290715) | November 10, 2015 | 11.0-CURRENT after API change to callout_stop macro; (rev [290664](https://svnweb.freebsd.org/changeset/base/290664)). |
...
| 1100096 | [294505](https://svnweb.freebsd.org/changeset/base/294505) | January 21, 2016 | 11.0-CURRENT after LinuxKPI idr_* additions. |
...
| 1100104 | [297156](https://svnweb.freebsd.org/changeset/base/297156) | March 21, 2016 | 11.0-CURRENT after tracking `filemon` usage via a proc.p_filemon pointer rather than its own lists. |
...
| 1100509 | [311186](https://svnweb.freebsd.org/changeset/base/311186) | January 3, 2017 | 11.0-STABLE after [crunchgen(1)](https://man.freebsd.org/cgi/man.cgi?query=crunchgen&sektion=1&format=html) META_MODE fix (rev [311185](https://svnweb.freebsd.org/changeset/base/311185)). |
...
| 1100513 | [318197](https://svnweb.freebsd.org/changeset/base/318197) | May 11, 2017 | 11.0-STABLE after merging the addition of the <dev/mmc/mmc_ioctl.h> header. |
...
| 1102502 | [338617](https://svnweb.freebsd.org/changeset/base/338617) | September 12, 2018 | 11-STABLE after adding a socket option SO_TS_CLOCK and fixing `recvmsg32()` system call to properly down-convert layout of the 64-bit structures to match what 32-bit app(s) expect. |
| 1102503 | [338931](https://svnweb.freebsd.org/changeset/base/338931) | September 25, 2018 | 11-STABLE after merging a TCP checksum fix to [iflib(9)](https://man.freebsd.org/cgi/man.cgi?query=iflib&sektion=9&format=html) and adding new media types to if_media.h |
| 1102504 | [340309](https://svnweb.freebsd.org/changeset/base/340309) | November 9, 2018 | 11-STABLE after several MFCs: updating [objcopy(1)](https://man.freebsd.org/cgi/man.cgi?query=objcopy&sektion=1&format=html) to properly handle little-endian MIPS64 object; correcting mips64el test to use ELF header; adding test for 64-bit ELF in _libelf_is_mips64el. |
...
| 1104506 | [365471](https://svnweb.freebsd.org/changeset/base/365471) | September 8, 2020 | 11-STABLE after adding atomic and `bswap` functions to libcompiler_rt. |
| 1104507 | [365661](https://svnweb.freebsd.org/changeset/base/365661) | September 12, 2020 | 11-STABLE after followup commits to libcompiler_rt. |
...
### 18.6. FreeBSD 10 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-10)

|     |     |     |     |
| --- | --- | --- | --- |Table 58. FreeBSD 10 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
| 1000007 | [231025](https://svnweb.freebsd.org/changeset/base/231025) | February 5, 2012 | 10-CURRENT after introduction of new [tcp(4)](https://man.freebsd.org/cgi/man.cgi?query=tcp&sektion=4&format=html) socket options: TCP_KEEPINIT, TCP_KEEPIDLE, TCP_KEEPINTVL, and TCP_KEEPCNT. |
| 1000008 | [231505](https://svnweb.freebsd.org/changeset/base/231505) | February 11, 2012 | 10-CURRENT after introduction of the new extensible [sysctl(3)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=3&format=html) interface NET_RT_IFLISTL to query address lists. |
...
| 1000019 | [241245](https://svnweb.freebsd.org/changeset/base/241245) | October 6, 2012 | 10-CURRENT after [pfil(9)](https://man.freebsd.org/cgi/man.cgi?query=pfil&sektion=9&format=html) KBI/KPI changed to supply packets in net byte order to AF_INET filter hooks. |
...
| 1000510 | [259067](https://svnweb.freebsd.org/changeset/base/259067) | December 7, 2013 | `releng/10.0` __FreeBSD_version update to prevent the value from going backwards. |
...
| 1000702 | [260135](https://svnweb.freebsd.org/changeset/base/260135) | December 31, 2013 | 10-STABLE after MAP_STACK fixes. |
...
| 1001513 | [281955](https://svnweb.freebsd.org/changeset/base/281955) | April 24, 2015 | 10-STABLE after starting the process of removing the use of the deprecated "M_FLOWID" flag from the network code. |
...
| 1003505 | [302228](https://svnweb.freebsd.org/changeset/base/302228) | June 27, 2016 | 10-STABLE after converting sed to use REG_STARTEND, fixing a Mesa issue. |
...
| 1003510 | [308047](https://svnweb.freebsd.org/changeset/base/308047) | October 28, 2016 | 10-STABLE after installing header files required development with libzfs_core. |
...
| 1003514 | [318198](https://svnweb.freebsd.org/changeset/base/318198) | May 11, 2017 | 10-STABLE after merging the addition of the <dev/mmc/mmc_ioctl.h> header. |
...
### 18.7. FreeBSD 9 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-9)

|     |     |     |     |
| --- | --- | --- | --- |Table 59. FreeBSD 9 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
| 900002 | [197430](https://svnweb.freebsd.org/changeset/base/197430) | September 23, 2009 | 9.0-CURRENT after implementing the EVFILT_USER `kevent` filter functionality. |
...
| 900017 | [211701](https://svnweb.freebsd.org/changeset/base/211701) | August 23, 2010 | 9.0-CURRENT after changing back default grep to GNU grep and adding WITH_BSD_GREP knob. |
| 900018 | [211735](https://svnweb.freebsd.org/changeset/base/211735) | August 24, 2010 | 9.0-CURRENT after the [pthread_kill(3)](https://man.freebsd.org/cgi/man.cgi?query=pthread_kill&sektion=3&format=html) -generated signal is identified as SI_LWP in `si_code`. Previously, `si_code` was SI_USER. |
| 900019 | [211937](https://svnweb.freebsd.org/changeset/base/211937) | August 28, 2010 | 9.0-CURRENT after addition of the MAP_PREFAULT_READ flag to [mmap(2)](https://man.freebsd.org/cgi/man.cgi?query=mmap&sektion=2&format=html). |
...
| 900024 | [215127](https://svnweb.freebsd.org/changeset/base/215127) | November 11, 2010 | 9.0-CURRENT after libgcc.a was replaced by libcompiler_rt.a. |
...
| 900026 | [216088](https://svnweb.freebsd.org/changeset/base/216088) | November 30, 2010 | 9.0-CURRENT after the introduction of Serial Management Protocol (SMP) passthrough and the XPT_SMP_IO and XPT_GDEV_ADVINFO CAM `CCBs`. |
...
| 900503 | [231768](https://svnweb.freebsd.org/changeset/base/231768) | February 15, 2012 | 9.0-STABLE after introduction of the new extensible [sysctl(3)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=3&format=html) interface NET_RT_IFLISTL to query address lists. |
...
| 900505 | [232945](https://svnweb.freebsd.org/changeset/base/232945) | March 13, 2012 | 9.0-STABLE after introduction of new [tcp(4)](https://man.freebsd.org/cgi/man.cgi?query=tcp&sektion=4&format=html) socket options: TCP_KEEPINIT, TCP_KEEPIDLE, TCP_KEEPINTVL, and TCP_KEEPCNT. |
...
| 901500 | [239081](https://svnweb.freebsd.org/changeset/base/239081) | August 6, 2012 | 9.1-STABLE after branching releng/9.1 (RELENG_9_1). |
...
| 902506 | [260136](https://svnweb.freebsd.org/changeset/base/260136) | December 31, 2013 | 9-STABLE after MAP_STACK fixes (rev [260082](https://svnweb.freebsd.org/changeset/base/260082)). |
...
### 18.8. FreeBSD 8 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-8)

|     |     |     |     |
| --- | --- | --- | --- |Table 60. FreeBSD 8 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
| 800008 | [174951](https://svnweb.freebsd.org/changeset/base/174951) | December 28, 2007 | 8.0-CURRENT after LK_EXCLUPGRADE option removal. |
...
| 800017 | [176015](https://svnweb.freebsd.org/changeset/base/176015) | February 6, 2008 | 8.0-CURRENT after the introduction of LK_NODUP and LK_NOWITNESS options in the [lockmgr(9)](https://man.freebsd.org/cgi/man.cgi?query=lockmgr&sektion=9&format=html) space. |
...
| 800021 | [176321](https://svnweb.freebsd.org/changeset/base/176321) | February 15, 2008 | 8.0-CURRENT after introduction of [lockmgr_args(9)](https://man.freebsd.org/cgi/man.cgi?query=lockmgr_args&sektion=9&format=html) and LK_INTERNAL flag removal. |
...
| 800025 | [176958](https://svnweb.freebsd.org/changeset/base/176958) | March 8, 2008 | 8.0-CURRENT after adding F_DUP2FD command to [fcntl(2)](https://man.freebsd.org/cgi/man.cgi?query=fcntl&sektion=2&format=html). |
...
| 800032 | [178006](https://svnweb.freebsd.org/changeset/base/178006) | April 8, 2008 | 8.0-CURRENT after the implementation of the `openat` and related syscalls, introduction of the O_EXEC flag for the [open(2)](https://man.freebsd.org/cgi/man.cgi?query=open&sektion=2&format=html), and providing the corresponding Linux compatibility syscalls. |
...
| 800048 | [183091](https://svnweb.freebsd.org/changeset/base/183091) | September 16, 2008 | 8.0-CURRENT after converting the kernel NFS mount code to accept individual mount options in the [nmount(2)](https://man.freebsd.org/cgi/man.cgi?query=nmount&sektion=2&format=html) `iovec`, not just one big struct nfs_args. |
...
| 800052 | [184419](https://svnweb.freebsd.org/changeset/base/184419) | October 28, 2008 | 8.0-CURRENT after the introduction of `accmode_t` and renaming of VOP_ACCESS `a_mode` argument to `a_accmode`. |
| 800053 | [184555](https://svnweb.freebsd.org/changeset/base/184555) | November 2, 2008 | 8.0-CURRENT after the prototype change of [vfs_busy(9)](https://man.freebsd.org/cgi/man.cgi?query=vfs_busy&sektion=9&format=html) and the introduction of its MBF_NOWAIT and MBF_MNTLSTLOCK flags. |
...
| 800058 | [185968](https://svnweb.freebsd.org/changeset/base/185968) | December 12, 2008 | 8.0-CURRENT after the introduction of the VOP_VPTOCNP operation. |
...
| 800072 | [189853](https://svnweb.freebsd.org/changeset/base/189853) | March 15, 2009 | 8.0-CURRENT after the IFF_NEEDSGIANT flag has been removed; non-MPSAFE network device drivers are no longer supported. |
...
| 800075 | [190787](https://svnweb.freebsd.org/changeset/base/190787) | April 6, 2009 | 8.0-CURRENT after layout of structs vnet_net, vnet_inet and vnet_ipfw has been changed. |
...
| 800077 | [190914](https://svnweb.freebsd.org/changeset/base/190914) | April 14, 2009 | 8.0-CURRENT after removing `VOP_LEASE()` and vop_vector.vop_lease. |
| 800078 | [191080](https://svnweb.freebsd.org/changeset/base/191080) | April 15, 2009 | 8.0-CURRENT after struct `rt_weight` fields have been added to struct `rt_metrics` and struct `rt_metrics_lite`, changing the layout of struct `rt_metrics_lite`. A bump to RTM_VERSION was made, but backed out. |
| 800079 | [191117](https://svnweb.freebsd.org/changeset/base/191117) | April 15, 2009 | 8.0-CURRENT after struct `llentry` pointers are added to struct route and struct route_in6. |
...
| 800106 | [195852](https://svnweb.freebsd.org/changeset/base/195852) | July 24, 2009 | 8.0-CURRENT after introduction of OBJT_SG VM object type. |
...
| 800108 | [199627](https://svnweb.freebsd.org/changeset/base/199627) | November 21, 2009 | 8.0-STABLE after implementing EVFILT_USER `kevent` filter. |
...
| 801501 | [214762](https://svnweb.freebsd.org/changeset/base/214762) | November 3, 2010 | 8.1-STABLE after KBI change in struct `sysentvec`, and implementation of PL_FLAG_SCE/SCX/EXEC/SI and `pl_siginfo` for ptrace(PT_LWPINFO) . |
...
| 802503 | [221275](https://svnweb.freebsd.org/changeset/base/221275) | May 1, 2011 | 8.2-STABLE after upgrade of the gcc to the last GPLv2 version from the FSF gcc-4_2-branch. |
...
| 802513 | [226763](https://svnweb.freebsd.org/changeset/base/226763) | October 25, 2011 | 8.2-STABLE after merging of the MAP_PREFAULT_READ flag to [mmap(2)](https://man.freebsd.org/cgi/man.cgi?query=mmap&sektion=2&format=html). |
...
| 802517 | [231769](https://svnweb.freebsd.org/changeset/base/231769) | February 15, 2012 | 8.2-STABLE after introduction of the new extensible [sysctl(3)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=3&format=html) interface NET_RT_IFLISTL to query address lists. |
| 803000 | [232446](https://svnweb.freebsd.org/changeset/base/232446) | March 3, 2012 | 8.3-RELEASE. |
| 803500 | [232439](https://svnweb.freebsd.org/changeset/base/232439) | March 3, 2012 | 8.3-STABLE after branching releng/8.3 (RELENG_8_3). |
...
### 18.9. FreeBSD 7 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-7)

|     |     |     |     |
| --- | --- | --- | --- |Table 61. FreeBSD 7 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
| 700000 | [147925](https://svnweb.freebsd.org/changeset/base/147925) | July 11, 2005 | 7.0-CURRENT. |
| 700001 | [148341](https://svnweb.freebsd.org/changeset/base/148341) | July 23, 2005 | 7.0-CURRENT after bump of all shared library versions that had not been changed since RELENG_5. |
...
| 700008 | [153107](https://svnweb.freebsd.org/changeset/base/153107) | December 5, 2005 | 7.0-CURRENT after removal of MNT_NODEV mount option. |
...
| 700012 | [154114](https://svnweb.freebsd.org/changeset/base/154114) | January 8, 2006 | 7.0-CURRENT after ldconfig_local_dirs change. |
...
| 700018 | [160386](https://svnweb.freebsd.org/changeset/base/160386) | July 15, 2006 | 7.0-CURRENT after import of snd_emu10kx. |
...
| 700030 | [166549](https://svnweb.freebsd.org/changeset/base/166549) | February 7, 2007 | 7.0-CURRENT after the removal of IPIP tunnel encapsulation (VIFF_TUNNEL) from the IPv4 multicast forwarding code. |
...
| 700043 | [169830](https://svnweb.freebsd.org/changeset/base/169830) | May 21, 2007 | 7.0-CURRENT after bump of all shared library versions that had not been changed since RELENG_6. |
...
| 700048 | [170613](https://svnweb.freebsd.org/changeset/base/170613) | June 12, 2007 | 7.0-CURRENT after RFC 3678 API support added to the IPv4 stack. Legacy RFC 1724 behavior of the IP_MULTICAST_IF ioctl has now been removed; 0.0.0.0/8 may no longer be used to specify an interface index. Use struct `ipmreqn` instead. |
| 700049 | [171175](https://svnweb.freebsd.org/changeset/base/171175) | July 3, 2007 | 7.0-CURRENT after importing pf from OpenBSD 4.1 |
| (not changed) | [171167](https://svnweb.freebsd.org/changeset/base/171167) |     | 7.0-CURRENT after adding IPv6 support for FAST_IPSEC, deleting KAME IPSEC, and renaming FAST_IPSEC to IPSEC. |
...
| 700107 | [178353](https://svnweb.freebsd.org/changeset/base/178353) | April 20, 2008 | 7.0-STABLE after the MFC of F_DUP2FD command to [fcntl(2)](https://man.freebsd.org/cgi/man.cgi?query=fcntl&sektion=2&format=html). |
...
| 701104 | [188625](https://svnweb.freebsd.org/changeset/base/188625) | February 14, 2009 | 7.1-STABLE after the store of the suspension owner in the struct mount, and introduction of vfs_susp_clean method into the struct vfsops. |
...
### 18.10. FreeBSD 6 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-6)

|     |     |     |     |
| --- | --- | --- | --- |Table 62. FreeBSD 6 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
| 600000 | [133921](https://svnweb.freebsd.org/changeset/base/133921) | August 18, 2004 | 6.0-CURRENT |
| 600001 | [134396](https://svnweb.freebsd.org/changeset/base/134396) | August 27, 2004 | 6.0-CURRENT after permanently enabling PFIL_HOOKS in the kernel. |
...
| 600016 | [142582](https://svnweb.freebsd.org/changeset/base/142582) | February 26, 2005 | 6.0-CURRENT after LC_CTYPE disk format changed. |
...
| 600018 | [142686](https://svnweb.freebsd.org/changeset/base/142686) | February 27, 2005 | 6.0-CURRENT after LC_COLLATE disk format changed. |
...
| 600020 | [143308](https://svnweb.freebsd.org/changeset/base/143308) | March 9, 2005 | Addition of MSG_NOSIGNAL flag to [send(2)](https://man.freebsd.org/cgi/man.cgi?query=send&sektion=2&format=html) API. |
...
| 600023 | [144980](https://svnweb.freebsd.org/changeset/base/144980) | April 13, 2005 | LOCAL_CREDS, LOCAL_CONNWAIT socket options added to [unix(4)](https://man.freebsd.org/cgi/man.cgi?query=unix&sektion=4&format=html). |
...
| 600033 | [148388](https://svnweb.freebsd.org/changeset/base/148388) | July 25, 2005 | 6.0-STABLE after bump of all shared library versions that had not been changed since RELENG_5. |
...
| 600104 | [154453](https://svnweb.freebsd.org/changeset/base/154453) | January 17, 2006 | 6.0-STABLE after MFC of ldconfig_local_dirs change. |
...
| 601105 | [161900](https://svnweb.freebsd.org/changeset/base/161900) | September 2, 2006 | 6.1-STABLE after 802.11 fix-ups changed the API for the IEEE80211_IOC_STA_INFO ioctl. |
...
| 602111 | [170556](https://svnweb.freebsd.org/changeset/base/170556) | June 11, 2007 | 6.2-STABLE after MFC of BOP_BDFLUSH and caused breakage of the filesystem modules KBI. |
...
### 18.11. FreeBSD 5 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-5)

|     |     |     |     |
| --- | --- | --- | --- |Table 63. FreeBSD 5 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
| 500015 | [70894](https://svnweb.freebsd.org/changeset/base/70894) | January 10, 2001 | 5.0-CURRENT after combining libgcc.a and libgcc_r.a, and associated GCC linkage changes. |
| 500016 | [71583](https://svnweb.freebsd.org/changeset/base/71583) | January 24, 2001 | 5.0-CURRENT after change allowing libc and libc_r to be linked together, deprecating -pthread option. |
...
| 500019 | [77937](https://svnweb.freebsd.org/changeset/base/77937) | June 9, 2001 | 5.0-CURRENT after moving machine/ioctl_fd.h to sys/fdcio.h |
...
| 500043 | [106289](https://svnweb.freebsd.org/changeset/base/106289) | November 1, 2002 | 5.0-CURRENT after adding weak pthread_XXX stubs to libc, obsoleting libXThrStub.so. 5.0-RELEASE. |
| 500100 | [109405](https://svnweb.freebsd.org/changeset/base/109405) | January 17, 2003 | 5.0-CURRENT after branching for RELENG_5_0 |
| 500101 | [111120](https://svnweb.freebsd.org/changeset/base/111120) | February 19, 2003 | <sys/dkstat.h> is empty. Do not include it. |
| 500102 | [111482](https://svnweb.freebsd.org/changeset/base/111482) | February 25, 2003 | 5.0-CURRENT after the d_mmap_t interface change. |
...
| 501100 | [115710](https://svnweb.freebsd.org/changeset/base/115710) | June 2, 2003 | 5.1-CURRENT after branching for RELENG_5_1. |
...
| 501108 | [120386](https://svnweb.freebsd.org/changeset/base/120386) | September 23, 2003 | 5.1-CURRENT after PFIL_HOOKS API update |
...
| 502100 | [123196](https://svnweb.freebsd.org/changeset/base/123196) | December 7, 2003 | 5.2-CURRENT after branching for RELENG_5_2 |
| 502101 | [123677](https://svnweb.freebsd.org/changeset/base/123677) | December 19, 2003 | 5.2-CURRENT after `*cxa_atexit*`*/*`cxa_finalize` functions were added to libc. |
| 502102 | [125236](https://svnweb.freebsd.org/changeset/base/125236) | January 30, 2004 | 5.2-CURRENT after change of default thread library from libc_r to libpthread. |
...
| 502118 | [130656](https://svnweb.freebsd.org/changeset/base/130656) | June 17, 2004 | 5.2-CURRENT after adding support for CLOCK_VIRTUAL and CLOCK_PROF to [clock_gettime(2)](https://man.freebsd.org/cgi/man.cgi?query=clock_gettime&sektion=2&format=html) and [clock_getres(2)](https://man.freebsd.org/cgi/man.cgi?query=clock_getres&sektion=2&format=html). |
...
| 502123 | [132025](https://svnweb.freebsd.org/changeset/base/132025) | July 12, 2004 | 5.2-CURRENT after change to make VFS_ROOT take a struct thread argument as does vflush. Struct `kinfo_proc` now has a user data pointer. The switch of the default X implementation to `xorg` was also made at this time. |
...
| 502128 | [133306](https://svnweb.freebsd.org/changeset/base/133306) | August 8, 2004 | 5.2-CURRENT after the change of the vfs_mount signature as well as global replacement of PRISON_ROOT with SUSER_ALLOWJAIL for the [suser(9)](https://man.freebsd.org/cgi/man.cgi?query=suser&sektion=9&format=html) API. |
...
| 503100 | [136595](https://svnweb.freebsd.org/changeset/base/136595) | October 16, 2004 | 5.3-STABLE after branching for RELENG_5_3 |
...
| 504100 | [144581](https://svnweb.freebsd.org/changeset/base/144581) | April 3, 2005 | 5.4-STABLE after branching for RELENG_5_4 |
...
| 504103 | [150892](https://svnweb.freebsd.org/changeset/base/150892) | October 3, 2005 | 5.4-STABLE after the MFC of if_bridge |
...
| 504105 | [154464](https://svnweb.freebsd.org/changeset/base/154464) | January 17, 2006 | 5.4-STABLE after MFC of ldconfig_local_dirs change. |
| 505000 | [158481](https://svnweb.freebsd.org/changeset/base/158481) | May 12, 2006 | 5.5-RELEASE. |
| 505100 | [158482](https://svnweb.freebsd.org/changeset/base/158482) | May 12, 2006 | 5.5-STABLE after branching for RELENG_5_5 |

### 18.12. FreeBSD 4 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-4)

|     |     |     |     |
| --- | --- | --- | --- |Table 64. FreeBSD 4 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
| 400008 | [48048](https://svnweb.freebsd.org/changeset/base/48048) | June 20, 1999 | 4.0-CURRENT after the addition of a poll syscall wrapper to libc_r |
...
| 420001 | [70895](https://svnweb.freebsd.org/changeset/base/70895) | January 10, 2001 | 4.2-STABLE after combining libgcc.a and libgcc_r.a, and associated GCC linkage changes. |
...
| 460102 | [102759](https://svnweb.freebsd.org/changeset/base/102759) | September 1, 2002 | 4.6-STABLE after MFC of many new pkg_install features from the HEAD. |
...
| 470102 | [109753](https://svnweb.freebsd.org/changeset/base/109753) | January 23, 2003 | 4.7-STABLE after MFC of mbuf changes to replace `m_aux` mbufs by `m_tag’s |
...
### 18.13. FreeBSD 3 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-3)

|     |     |     |     |
| --- | --- | --- | --- |Table 65. FreeBSD 3 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
### 18.14. FreeBSD 2.2 Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-2.2)

|     |     |     |     |
| --- | --- | --- | --- |Table 66. FreeBSD 2.2 `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
...
### 18.15. FreeBSD 2 Before 2.2-RELEASE Versions[](https://docs.freebsd.org/en/books/porters-handbook/book/#versions-2)

|     |     |     |     |
| --- | --- | --- | --- |Table 67. FreeBSD 2 Before 2.2-RELEASE `__FreeBSD_version` Values
| Value | Revision | Date | Release |
| --- | --- | --- | --- |
| 119411 |     |     | 2.0-RELEASE |
